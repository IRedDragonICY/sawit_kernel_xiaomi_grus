...
static int proc_taint(struct ctl_table *table, int write, void __user *buffer,
...
static int bpf_unpriv_handler(struct ctl_table *table, int write, void *buffer,
                              size_t *lenp, loff_t *ppos) {
...
static int _proc_do_string(char *data, int maxlen, int write,
                           char __user *buffer, size_t *lenp, loff_t *ppos) {
  size_t len;
  char __user *p;
  char c;
 ...
 */
static int proc_get_long(char **buf, size_t *size, unsigned long *val,
                         bool *neg, const char *perm_tr, unsigned perm_tr_len,
                         char *tr) {
...
static int do_proc_dointvec_conv(bool *negp, unsigned long *lvalp, int *valp,
                                 int write, void *data) {
  if (write) {
    if (*negp) {
...
static int do_proc_douintvec_conv(bool *negp, unsigned long *lvalp, int *valp,
                                  int write, void *data) {
  if (write) {
    if (*negp)
      return -EINVAL;
...
static int __do_proc_dointvec(void *tbl_data, struct ctl_table *table,
  ...
  for (; left && vleft--; i++, first = 0) {
    ...
    if (write) {
      ...
      if (!left)
        break;
      err = proc_get_long(&p, &left, &lval, &neg, proc_wspace_sep,
...
static int do_proc_dointvec(struct ctl_table *table, int write,
                            ...
                            int (*conv)(bool *negp, unsigned long *lvalp,
                                        int *valp, int write, void *data),
                            void *data) {
  return __do_proc_dointvec(table->data, table, write, buffer, lenp, ppos, conv,
                            data);
}
 ...
 * Returns 0 on success.
 */
int proc_douintvec(struct ctl_table *table, int write, void __user *buffer,
                   size_t *lenp, loff_t *ppos) {
...
static int do_proc_dointvec_minmax_conv(bool *negp, unsigned long *lvalp,
  ...
  if (write) {
    ...
    if ((param->min && *param->min > val) || (param->max && *param->max < val))
      return -EINVAL;
    *valp = val;
  } else {
    int val = *valp;
...
static int __do_proc_doulongvec_minmax(void *data, struct ctl_table *table,
  ...
  for (; left && vleft--; i++, first = 0) {
    ...
    if (write) {
      ...
      err = proc_get_long(&p, &left, &val, &neg, proc_wspace_sep,
                          sizeof(proc_wspace_sep), NULL);
      if (err)
        break;
      if (neg)
...
static int do_proc_doulongvec_minmax(struct ctl_table *table, int write,
                                     void __user *buffer, size_t *lenp,
                                     loff_t *ppos, unsigned long convmul,
                                     unsigned long convdiv) {
  return __do_proc_doulongvec_minmax(table->data, table, write, buffer, lenp,
...
/**
 * proc_doulongvec_ms_jiffies_minmax - read a vector of millisecond values with
 * min/max values
 * @table: the sysctl table
 * @write: %TRUE if this is a write to the sysctl file
 * @buffer: the user buffer
...
int proc_doulongvec_ms_jiffies_minmax(struct ctl_table *table, int write,
                                      void __user *buffer, size_t *lenp,
                                      loff_t *ppos) {
  return do_proc_doulongvec_minmax(table, write, buffer, lenp, ppos, HZ, 1000l);
}
...
static int do_proc_dointvec_userhz_jiffies_conv(bool *negp,
                                                unsigned long *lvalp, int *valp,
                                                int write, void *data) {
  if (write) {
...
static int proc_do_cad_pid(struct ctl_table *table, int write,
  ...
  r = __do_proc_dointvec(&tmp, table, write, buffer, lenp, ppos, NULL, NULL);
  if (r || !write)
    return r;
...
int proc_do_large_bitmap(struct ctl_table *table, int write,
  ...
  if (write) {
    ...
    tmp_bitmap =
        kzalloc(BITS_TO_LONGS(bitmap_len) * sizeof(unsigned long), GFP_KERNEL);
    if (!tmp_bitmap) {
      kfree(kbuf);
      return -ENOMEM;
    }
    ...
    while (!err && left) {
      ...
      err = proc_get_long(&p, &left, &val_a, &neg, tr_a, sizeof(tr_a), &c);
      if (err)
        break;
      ...
      if (c == '-') {
        err = proc_get_long(&p, &left, &val_b, &neg, tr_b, sizeof(tr_b), &c);
        if (err)
          break;
    ...
    while (left) {
      ...
      if (bit_a >= bitmap_len)
        break;
      bit_b = find_next_zero_bit(bitmap, bitmap_len, bit_a + 1) - 1;
...
#if defined(CONFIG_SYSCTL)
int proc_do_static_key(struct ctl_table *table, int write, void __user *buffer,
                       size_t *lenp, loff_t *ppos) {
