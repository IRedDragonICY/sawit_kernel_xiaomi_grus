...
static inline void walt_inc_cfs_rq_stats(struct cfs_rq *cfs_rq,
                                         struct task_struct *p) {}
static inline void walt_dec_cfs_rq_stats(struct cfs_rq *cfs_rq,
                                         struct task_struct *p) {}
...
int sched_proc_update_handler(struct ctl_table *table, int write,
  ...
  sched_nr_latency =
      DIV_ROUND_UP(sysctl_sched_latency, sysctl_sched_min_granularity);
  ...
  if (block_start) {
    ...
    if (tsk) {
      ...
      if (unlikely(prof_on == SLEEP_PROFILING)) {
        profile_hits(SLEEP_PROFILING, (void *)get_wchan(tsk), delta >> 20);
      }
...
static inline unsigned long group_faults_cpu(struct numa_group *group,
                                             int nid) {
 ...
 */
static void task_numa_compare(struct task_numa_env *env, long taskimp,
                              long groupimp) {
  ...
  if (imp > env->best_imp && src_rq->nr_running == 1 && dst_rq->nr_running == 1)
    goto assign;
...
balance:
  ...
  if (!cur) {
     ...
     */
    local_irq_disable();
    env->dst_cpu = select_idle_sibling(env->p, env->src_cpu, env->dst_cpu);
    local_irq_enable();
  }
...
static int task_numa_migrate(struct task_struct *p) {
   ...
   */
  if (env.best_cpu == -1 ||
      (p->numa_group && p->numa_group->active_nodes > 1)) {
...
static void update_task_scan_period(struct task_struct *p, unsigned long shared,
  ...
  p->numa_scan_period =
      clamp(p->numa_scan_period + diff, task_scan_min(p), task_scan_max(p));
...
static void task_numa_group(struct task_struct *p, int cpupid, int flags,
  ...
  if (unlikely(!p->numa_group)) {
    unsigned int size =
        sizeof(struct numa_group) + 4 * nr_node_ids * sizeof(unsigned long);
...
void task_numa_fault(int last_cpupid, int mem_node, int pages, int flags) {
  ...
  if (unlikely(!p->numa_faults)) {
    int size =
        sizeof(*p->numa_faults) * NR_NUMA_HINT_FAULT_BUCKETS * nr_node_ids;
  ...
  if (!priv && !local && ng && ng->active_nodes > 1 &&
      numa_is_active_node(cpu_node, ng) && numa_is_active_node(mem_node, ng))
    local = 1;
...
void task_numa_work(struct callback_head *work) {
  ...
  if (!mm->numa_next_scan) {
    mm->numa_next_scan =
        now + msecs_to_jiffies(sysctl_numa_balancing_scan_delay);
  }
...
void task_tick_numa(struct rq *rq, struct task_struct *curr) {
  ...
  if (now > curr->node_stamp + period) {
    ...
    if (!time_before(jiffies, curr->mm->numa_next_scan)) {
      init_task_work(work,
                     task_numa_work); /* TODO: move this into sched_fork() */
...
#else  /* CONFIG_SMP */
static inline long calc_cfs_shares(struct cfs_rq *cfs_rq,
...
void set_task_rq_fair(struct sched_entity *se, struct cfs_rq *prev,
  ...
  if (se->avg.last_update_time && prev) {
    ...
    n_last_update_time = next->avg.last_update_time;
#endif
    __update_load_avg(p_last_update_time, cpu_of(rq_of(prev)), &se->avg, 0, 0,
                      NULL);
...
static inline void check_schedstat_required(void) {
  ...
  if (trace_sched_stat_wait_enabled() || trace_sched_stat_sleep_enabled() ||
      trace_sched_stat_iowait_enabled() || trace_sched_stat_blocked_enabled() ||
...
static void set_next_entity(struct cfs_rq *cfs_rq, struct sched_entity *se) {
   ...
   */
  if (schedstat_enabled() &&
      rq_of(cfs_rq)->load.weight >= 2 * se->load.weight) {
...
static struct sched_entity *pick_next_entity(struct cfs_rq *cfs_rq,
  ...
  if (cfs_rq->skip == se) {
    ...
    if (second && (strict_skip || wakeup_preempt_entity(second, left) < 1))
      se = second;
  }
...
static bool cfs_bandwidth_used(void) { return true; }
...
static void throttle_cfs_rq(struct cfs_rq *cfs_rq) {
  ...
  for_each_sched_entity(se) {
    ...
    walt_propagate_cumulative_runnable_avg(&qcfs_rq->cumulative_runnable_avg,
  ...
  if (!se) {
    ...
    walt_propagate_cumulative_runnable_avg(
        &rq->cumulative_runnable_avg, cfs_rq->cumulative_runnable_avg, false);
  }
...
void unthrottle_cfs_rq(struct cfs_rq *cfs_rq) {
  ...
  for_each_sched_entity(se) {
    ...
    walt_propagate_cumulative_runnable_avg(&cfs_rq->cumulative_runnable_avg,
                                           tcfs_rq->cumulative_runnable_avg,
                                           true);
  ...
  if (!se) {
    ...
    walt_propagate_cumulative_runnable_avg(
        &rq->cumulative_runnable_avg, tcfs_rq->cumulative_runnable_avg, true);
  }
...
static void start_cfs_slack_bandwidth(struct cfs_bandwidth *cfs_b) {
  ...
  hrtimer_start(&cfs_b->slack_timer, ns_to_ktime(cfs_bandwidth_slack_period),
                HRTIMER_MODE_REL);
}
  ...
  for (;;) {
    ...
    if (++count > 3) {
      ...
      if (new < max_cfs_quota_period) {
        ...
        pr_warn_ratelimited("cfs_period_timer[cpu%d]: period too short, but "
                            "cannot scale up without losing precision "
                            "(cfs_period_us = %lld, cfs_quota_us = %lld)\n",
...
static inline int throttled_hierarchy(struct cfs_rq *cfs_rq) { return 0; }
...
static void enqueue_task_fair(struct rq *rq, struct task_struct *p, int flags) {
  ...
  if (!se) {
    if (!task_new && !rq->rd->overutilized && cpu_overutilized(rq->cpu)) {
...
static void cpu_load_update_nohz(struct rq *this_rq, unsigned long curr_jiffies,
                                 unsigned long load) {
  unsigned long pending_updates;
 ...
 */
bool sched_is_energy_aware(void) { return energy_aware(); }
...
unsigned long capacity_curr_of(int cpu) {
  return cpu_rq(cpu)->cpu_capacity_orig * arch_scale_freq_capacity(NULL, cpu) >>
 ...
 */
static unsigned long __cpu_norm_util(unsigned long util,
                                     unsigned long capacity) {
  if (util >= capacity)
...
static int group_idle_state(struct energy_env *eenv, int cpu_idx) {
  ...
  for_each_cpu(i, sched_group_cpus(sg)) state =
      min(state, idle_get_state_idx(cpu_rq(i)));
  ...
  src_in_grp =
      cpumask_test_cpu(eenv->cpu[EAS_CPU_PRV].cpu_id, sched_group_cpus(sg));
  dst_in_grp =
      cpumask_test_cpu(eenv->cpu[cpu_idx].cpu_id, sched_group_cpus(sg));
  ...
  if (grp_util <= ((long)sg->sgc->max_capacity * (int)sg->group_weight)) {
    ...
    if (grp_util <= 0)
       ...
       */
      new_state =
          min(max_idle_state_idx, (int)(new_state / sg->sgc->max_capacity));
...
static inline int select_energy_cpu_idx(struct energy_env *eenv) {
  ...
  trace_sched_energy_diff(
      eenv->p, eenv->cpu[EAS_CPU_PRV].cpu_id, eenv->cpu[EAS_CPU_PRV].energy,
      eenv->cpu[EAS_CPU_NXT].cpu_id, eenv->cpu[EAS_CPU_NXT].energy,
      eenv->cpu[EAS_CPU_BKP].cpu_id, eenv->cpu[EAS_CPU_BKP].energy);
  /*
  ...
  for (cpu_idx = EAS_CPU_NXT; cpu_idx < EAS_CPU_CNT; ++cpu_idx) {
    ...
    eenv->cpu[cpu_idx].nrg_delta =
        eenv->cpu[cpu_idx].energy - eenv->cpu[EAS_CPU_PRV].energy;
    ...
    if (eenv->cpu[cpu_idx].nrg_delta < eenv->cpu[eenv->next_idx].nrg_delta) {
...
static inline bool cpu_check_overutil_condition(int cpu, unsigned long util) {
...
bool cpu_overutilized(int cpu) { return __cpu_overutilized(cpu, 0); }
...
static inline int schedtune_cpu_margin(unsigned long util, int cpu) {
...
static inline long schedtune_task_margin(struct task_struct *p) {
...
static inline int schedtune_task_margin(struct task_struct *p) { return 0; }

#endif /* CONFIG_SCHED_TUNE */

unsigned long boosted_cpu_util(int cpu, struct sched_walt_cpu_load *walt_load) {
 ...
 */
static struct sched_group *find_idlest_group(struct sched_domain *sd,
                                             struct task_struct *p,
  ...
  unsigned long imbalance =
      scale_load_down(NICE_0_LOAD) * (sd->imbalance_pct - 100) / 100;
  ...
  do {
    ...
    if (!cpumask_intersects(sched_group_cpus(group), tsk_cpus_allowed(p)))
      continue;
...
skip_spare:
  if (!idlest || (min_runnable_load > (this_runnable_load + imbalance)) ||
...
static inline int select_idle_core(struct task_struct *p,
...
static int select_idle_sibling(struct task_struct *p, int prev, int target) {
  ...
  for_each_lower_domain(sd) {
    sg = sd->groups;
    do {
      if (!cpumask_intersects(sched_group_cpus(sg), tsk_cpus_allowed(p)))
        goto next;
      ...
      if (sysctl_sched_cstate_aware) {
        ...
        target = cpumask_first_and(sched_group_cpus(sg), tsk_cpus_allowed(p));
...
static bool is_packing_eligible(struct task_struct *p, int target_cpu,
  ...
  estimated_capacity = add_capacity_margin(estimated_capacity, target_cpu);
...
static int start_cpu(struct task_struct *p, bool boosted,
  ...
  if (rtg_target) {
    int rtg_target_cpu = cpumask_first_and(rtg_target, cpu_online_mask);
  ...
  if (rd->min_cap_orig_cpu != -1 && task_fits_max(p, rd->min_cap_orig_cpu))
...
static inline int find_best_target(struct task_struct *p, int *backup_cpu,
  ...
  *backup_cpu = -1;
  ...
  do {
    ...
    i = find_first_cpu_bit(p, &search_cpus, sg, &avoid_prev_cpu, &do_rotate,
                           &first_cpu_bit_env);
    if (do_rotate)
  ...
  retry:
    while ((i = cpumask_next(i, &search_cpus)) < nr_cpu_ids) {
      ...
      if (prefer_idle) {
        ...
        if (idle_cpu(i)) {
          ...
          trace_sched_find_best_target(p, prefer_idle, min_util, cpu,
                                       best_idle_cpu, best_active_cpu, i, -1);
      ...
      if (idle_cpu(i)) {
         ...
         */
        if (sysctl_sched_cstate_aware && best_idle_cstate <= idle_idx)
          continue;

        /* Keep track of best idle CPU */
        target_capacity = capacity_orig;
        target_idle_max_spare_cap = capacity_orig - min_capped_util;
        best_idle_cstate = idle_idx;
        best_idle_cpu = i;
        continue;
      }
      ...
      if ((capacity_orig - min_capped_util) < target_max_spare_cap)
        continue;
    ...
    if (!sysctl_sched_is_big_little && !prefer_idle) {
       ...
       */
      if (target_cpu != -1 && target_util < sched_smp_overlap_capacity)
        break;
   ...
   */
  if (target_cpu == -1)
    target_cpu = prefer_idle ? best_active_cpu : best_idle_cpu;
  else
    *backup_cpu = prefer_idle ? best_active_cpu : best_idle_cpu;
  ...
  trace_sched_find_best_target(p, prefer_idle, min_util, cpu, best_idle_cpu,
...
static inline int wake_to_idle(struct task_struct *p) {
  return (current->flags & PF_WAKE_UP_IDLE) || (p->flags & PF_WAKE_UP_IDLE);
}

static inline bool bias_to_waker_cpu(struct task_struct *p, int cpu,
                                     struct cpumask *rtg_target) {
  int rtg_target_cpu = rtg_target ? cpumask_first(rtg_target) : cpu;

  return cpumask_test_cpu(cpu, tsk_cpus_allowed(p)) && cpu_active(cpu) &&
         !cpu_isolated(cpu) &&
...
static inline bool bias_to_prev_cpu(struct task_struct *p,
                                    struct cpumask *rtg_target) {
  int prev_cpu = task_cpu(p);
#ifdef CONFIG_SCHED_WALT
...
static int select_energy_cpu_brute(struct task_struct *p, int prev_cpu,
  ...
  next_cpu = find_best_target(p, &backup_cpu, boosted, prefer_idle, &fbt_env);
  if (next_cpu == -1) {
    target_cpu = prev_cpu;
    goto out;
  }

  if (fbt_env.placement_boost || fbt_env.need_idle || fbt_env.avoid_prev_cpu ||
...
static void set_last_buddy(struct sched_entity *se) {
  ...
  for_each_sched_entity(se) cfs_rq_of(se)->last = se;
}
...
static void set_next_buddy(struct sched_entity *se) {
  ...
  for_each_sched_entity(se) cfs_rq_of(se)->next = se;
}
 ...
 */
static int migrate_degrades_locality(struct task_struct *p,
                                     struct lb_env *env) {
...
static int can_migrate_task(struct task_struct *p, struct lb_env *env) {
  ...
  if (energy_aware() && !env->dst_rq->rd->overutilized &&
      env->idle == CPU_NEWLY_IDLE && !task_in_related_thread_group(p)) {
  ...
  if (env->flags & LBF_IGNORE_BIG_TASKS && !task_fits_max(p, env->dst_cpu))
    return 0;
#endif
...
redo:
  ...
  if (env->flags &
          (LBF_IGNORE_BIG_TASKS | LBF_IGNORE_PREFERRED_CLUSTER_TASKS) &&
      !detached) {
    tasks = &env->src_rq->cfs_tasks;
    env->flags &= ~(LBF_IGNORE_BIG_TASKS | LBF_IGNORE_PREFERRED_CLUSTER_TASKS);
    env->loop = orig_loop;
    goto redo;
  }
 ...
 */
static inline bool group_has_capacity(struct lb_env *env,
 ...
 *  false.
 */
static inline bool group_is_overloaded(struct lb_env *env,
                                       struct sg_lb_stats *sgs) {
  if (sgs->sum_nr_running <= sgs->group_weight)
    return false;
 ...
 */
static inline bool group_smaller_cpu_capacity(struct sched_group *sg,
                                              struct sched_group *ref) {
  return sg->sgc->max_capacity < ref->sgc->max_capacity;
}

static inline enum group_type group_classify(struct sched_group *group,
                                             struct sg_lb_stats *sgs) {
...
static inline void update_sg_lb_stats(struct lb_env *env,
  ...
  for_each_cpu_and(i, sched_group_cpus(group), env->cpus) {
    ...
    trace_sched_cpu_load_lb(cpu_rq(i), idle_cpu(i), sched_irqload(i),
                            power_cost(i, 0));
...
static inline enum fbq_type fbq_classify_rq(struct rq *rq) { return regular; }
#endif /* CONFIG_NUMA_BALANCING */
 ...
 */
static inline void fix_small_imbalance(struct lb_env *env,
                                       struct sd_lb_stats *sds) {
  ...
  scaled_busy_load_per_task =
      (busiest->load_per_task * SCHED_CAPACITY_SCALE) / busiest->group_capacity;
  ...
  capa_now +=
      busiest->group_capacity * min(busiest->load_per_task, busiest->avg_load);
  capa_now +=
      local->group_capacity * min(local->load_per_task, local->avg_load);
  ...
  if (busiest->avg_load * busiest->group_capacity <
    ...
    tmp = (busiest->avg_load * busiest->group_capacity) / local->group_capacity;
  } else {
    tmp =
        (busiest->load_per_task * SCHED_CAPACITY_SCALE) / local->group_capacity;
  }
  capa_move +=
      local->group_capacity * min(local->load_per_task, local->avg_load + tmp);
...
static inline void calculate_imbalance(struct lb_env *env,
  ...
  env->imbalance =
      min(max_pull * busiest->group_capacity,
          (sds->avg_load - local->avg_load) * local->group_capacity) /
      SCHED_CAPACITY_SCALE;

  /* Boost imbalance to allow misfit task to be balanced. */
  if (busiest->group_type == group_misfit_task)
    env->imbalance = max_t(long, env->imbalance, busiest->group_misfit_task);
  ...
  if (env->imbalance < busiest->load_per_task) {
     ...
     */
    if (busiest->group_type == group_overloaded &&
        local->group_type <= group_misfit_task && env->idle != CPU_NOT_IDLE) {
      env->imbalance = busiest->load_per_task;
      return;
    }
...
static struct rq *find_busiest_queue(struct lb_env *env,
  ...
  for_each_cpu_and(i, sched_group_cpus(group), env->cpus) {
    ...
    if (energy_aware() &&
        ...
        rq->misfit_task && env->busiest_group_type == group_misfit_task) {
      busiest_load = wl;
...
static int should_we_balance(struct lb_env *env) {
  ...
  for_each_cpu_and(cpu, sg_cpus, env->cpus) {
    if (!cpumask_test_cpu(cpu, sg_mask) || !idle_cpu(cpu) || cpu_isolated(cpu))
      continue;
...
no_move:
  if (!ld_moved) {
     ...
     */
    if (idle != CPU_NEWLY_IDLE && !(env.flags & LBF_BIG_TASK_ACTIVE_BALANCE)) {
    ...
    if (need_active_balance(&env)) {
       ...
       */
      if (is_reserved(this_cpu) || is_reserved(cpu_of(busiest))) {
        raw_spin_unlock_irqrestore(&busiest->lock, flags);
        *continue_balancing = 0;
        goto out;
      }
      ...
      sd->nr_balance_failed =
          sd->cache_nice_tries + NEED_ACTIVE_BALANCE_THRESHOLD - 1;
    }
  } else
...
#else
static inline bool min_cap_cluster_has_misfit_task(void) { return false; }
#endif
...
static int idle_balance(struct rq *this_rq) {
  ...
  for_each_domain(this_cpu, sd) {
    ...
    if (!force_lb && this_rq->avg_idle < curr_cost + sd->max_newidle_lb_cost) {
      update_next_balance(sd, &next_balance);
      break;
    }
...
static int active_load_balance_cpu_stop(void *data) {
  ...
  if (push_task) {
    if (task_on_rq_queued(push_task) && push_task->state == TASK_RUNNING &&
        task_cpu(push_task) == busiest_cpu && cpu_online(target_cpu)) {
...
static inline int find_new_ilb(int type) {
  ...
  if (sd) {
    if (energy_aware() && rq->misfit_task)
      cpumask_andnot(&cpumask, nohz.idle_cpus_mask, sched_domain_span(sd));
    else
      cpumask_and(&cpumask, nohz.idle_cpus_mask, sched_domain_span(sd));
    cpumask_andnot(&cpumask, &cpumask, cpu_isolated_mask);
  ...
  if (sd && (ilb >= nr_cpu_ids || !idle_cpu(ilb))) {
    if (!energy_aware() ||
        (capacity_orig_of(cpu) == cpu_rq(cpu)->rd->max_cpu_capacity.val ||
...
static void nohz_idle_balance(struct rq *this_rq, enum cpu_idle_type idle) {
  ...
  if (idle != CPU_IDLE || !test_bit(NOHZ_BALANCE_KICK, nohz_flags(this_cpu)))
    goto end;
...
static inline bool nohz_kick_needed(struct rq *rq, int *type) {
  ...
  if (sds && !energy_aware()) {
     ...
     * See also:
     * http://lkml.kernel.org/r/20111202010832.602203411@sbsiddha-desk.sc.intel.com
     */
...
#ifdef CONFIG_SCHED_WALT
static inline void walt_update_misfit_task(struct rq *rq,
...
void print_cfs_stats(struct seq_file *m, int cpu) {
  ...
  rcu_read_lock();
  for_each_leaf_cfs_rq(cpu_rq(cpu), cfs_rq) print_cfs_rq(m, cpu, cfs_rq);
  rcu_read_unlock();
}
...
static int task_will_be_throttled(struct task_struct *p) { return false; }

#endif /* CONFIG_CFS_BANDWIDTH */

static inline int kick_active_balance(struct rq *rq, struct task_struct *p,
                                      int new_cpu) {
  unsigned long flags;
  int rc = 0;
...
static void walt_rotate_work_func(struct work_struct *work) {
  struct walt_rotate_work *wr = container_of(work, struct walt_rotate_work, w);
...
static void walt_check_for_rotation(struct rq *src_rq) {
  ...
  for_each_possible_cpu(i) {
    ...
    if (!rq->misfit_task || rq->curr->sched_class != &fair_sched_class)
      continue;
