 ...
 */
/*
 * BACKPORT:[Cursor] KGSL Idle Opt - Reduce minimum evaluation
 * interval from 5ms to 2ms. Combined with governor polling
 * interval reduction, this lets the TZ algorithm react to idle
 * conditions 2.5x faster.
 */
#define FLOOR 2000
/*
...
static DEVICE_ATTR(suspend_time, 0444, suspend_time_show, NULL);

static const struct device_attribute *adreno_tz_attr_list[] = {
    &dev_attr_gpu_load, &dev_attr_suspend_time, NULL};
...
static int __secure_tz_reset_entry2(unsigned int *scm_data, u32 size_scm_data,
  ...
  if (!is_64) {
    spin_lock(&tz_lock);
    ret = scm_call_atomic2(SCM_SVC_IO, TZ_RESET_ID, scm_data[0], scm_data[1]);
    spin_unlock(&tz_lock);
  } else {
    if (is_scm_armv8()) {
      ...
      desc.arginfo = 0;
      ret = scm_call2(SCM_SIP_FNID(SCM_SVC_DCVS, TZ_RESET_ID_64), &desc);
    } else {
      ret = scm_call(SCM_SVC_DCVS, TZ_RESET_ID_64, scm_data, size_scm_data,
                     NULL, 0);
    }
  }
  return ret;
...
static int __secure_tz_update_entry3(unsigned int *scm_data, u32 size_scm_data,
  ...
  if (!priv->is_64) {
    spin_lock(&tz_lock);
    ret = scm_call_atomic3(SCM_SVC_IO, TZ_UPDATE_ID, scm_data[0], scm_data[1],
                           scm_data[2]);
    spin_unlock(&tz_lock);
    *val = ret;
  } else {
    if (is_scm_armv8()) {
      ...
      *val = desc.ret[0];
      ...
      ret = scm_call(SCM_SVC_DCVS, TZ_UPDATE_ID_64, scm_data, size_scm_data,
                     val, size_val);
    }
  }
  return ret;
...
static int tz_init_ca(struct devfreq_msm_adreno_tz_data *priv) {
  ...
  mb();
  dmac_flush_range(tz_buf, tz_buf + PAGE_ALIGN(sizeof(tz_ca_data)));

  desc.args[0] = virt_to_phys(tz_buf);
  desc.args[1] = sizeof(tz_ca_data);
  desc.arginfo = SCM_ARGS(2, SCM_RW, SCM_VAL);

  ret = scm_call2(SCM_SIP_FNID(SCM_SVC_DCVS, TZ_V2_INIT_CA_ID_64), &desc);
...
static int tz_init(struct devfreq_msm_adreno_tz_data *priv,
  ...
  if (scm_is_call_available(SCM_SVC_DCVS, TZ_INIT_ID)) {
    ret = scm_call(SCM_SVC_DCVS, TZ_INIT_ID, tz_pwrlevels, size_pwrlevels, NULL,
                   0);
    *version = 0;
    ...
    if (!is_scm_armv8()) {
      ret = scm_call(SCM_SVC_DCVS, TZ_INIT_ID_64, tz_pwrlevels, size_pwrlevels,
    ...
    ret = scm_call2(SCM_SIP_FNID(SCM_SVC_DCVS, TZ_V2_INIT_ID_64), &desc);
    *version = desc.ret[0];
    if (!ret)
      priv->is_64 = true;
    kzfree(tz_buf);
  ...
  if (!ret && priv->ctxt_aware_enable) {
    if (priv->is_64 &&
        (scm_is_call_available(SCM_SVC_DCVS, TZ_V2_INIT_CA_ID_64)) &&
        (scm_is_call_available(SCM_SVC_DCVS, TZ_V2_UPDATE_WITH_CA_ID_64))) {
      ret = tz_init_ca(priv);
      /*
...
static int tz_get_target_freq(struct devfreq *devfreq, unsigned long *freq) {
  ...
  *freq = stats.current_frequency;
  ...
  /*
   * BACKPORT:[Cursor] KGSL Idle Opt - Aggressive idle frequency
   * drop. When the GPU has been idle for longer than CEILING but
   * busy_time is below MIN_BUSY, force the frequency to the
   * lowest available level instead of doing nothing. This
   * prevents the GPU from sitting at a high frequency while
   * effectively idle.
   */
  if ((unsigned int)priv->bin.busy_time < MIN_BUSY) {
    if (stats.total_time > CEILING) {
  ...
  if (!priv->disable_busy_time_burst && priv->bin.busy_time > CEILING) {
    ...
    __secure_tz_update_entry3(scm_data, sizeof(scm_data), &val, sizeof(val),
                              priv);
  }
  priv->bin.total_time = 0;
  priv->bin.busy_time = 0;
...
static int tz_notify(struct notifier_block *nb, unsigned long type,
                     void *devp) {
  int result = 0;
...
static int tz_start(struct devfreq *devfreq) {
  ...
  struct msm_adreno_extended_profile *gpu_profile = container_of(
      (devfreq->profile), struct msm_adreno_extended_profile, profile);
  ...
  INIT_WORK(&gpu_profile->partner_start_event_ws, do_partner_start_event);
  INIT_WORK(&gpu_profile->partner_stop_event_ws, do_partner_stop_event);
  INIT_WORK(&gpu_profile->partner_suspend_event_ws, do_partner_suspend_event);
  INIT_WORK(&gpu_profile->partner_resume_event_ws, do_partner_resume_event);
...
static int tz_handler(struct devfreq *devfreq, unsigned int event, void *data) {
  ...
  case DEVFREQ_GOV_STOP:
    ...
    if (partner_gpu_profile && partner_gpu_profile->bus_devfreq)
      queue_work(workqueue, &gpu_profile->partner_stop_event_ws);
    spin_lock(&suspend_lock);
    suspend_start = 0;
    spin_unlock(&suspend_lock);
  ...
  if (partner_gpu_profile && partner_gpu_profile->bus_devfreq)
    switch (event) {
    case DEVFREQ_GOV_START:
      queue_work(workqueue, &gpu_profile->partner_start_event_ws);
      break;
    case DEVFREQ_GOV_SUSPEND:
      queue_work(workqueue, &gpu_profile->partner_suspend_event_ws);
      break;
    case DEVFREQ_GOV_RESUME:
      queue_work(workqueue, &gpu_profile->partner_resume_event_ws);
      break;
    }
...
static void _do_partner_event(struct work_struct *work, unsigned int event) {
  ...
  if (bus_devfreq != NULL && bus_devfreq->governor &&
