...
#define SDE_DEBUG_ENC(e, fmt, ...)                                             \
  SDE_DEBUG("enc%d " fmt, (e) ? (e)->base.base.id : -1, ##__VA_ARGS__)

#define SDE_ERROR_ENC(e, fmt, ...)                                             \
  SDE_ERROR("enc%d " fmt, (e) ? (e)->base.base.id : -1, ##__VA_ARGS__)

#define SDE_DEBUG_PHYS(p, fmt, ...)                                            \
  SDE_DEBUG("enc%d intf%d pp%d " fmt, (p) ? (p)->parent->base.id : -1,         \
            (p) ? (p)->intf_idx - INTF_0 : -1,                                 \
            (p) ? ((p)->hw_pp ? (p)->hw_pp->idx - PINGPONG_0 : -1) : -1,       \
            ##__VA_ARGS__)

#define SDE_ERROR_PHYS(p, fmt, ...)                                            \
  SDE_ERROR("enc%d intf%d pp%d " fmt, (p) ? (p)->parent->base.id : -1,         \
            (p) ? (p)->intf_idx - INTF_0 : -1,                                 \
            (p) ? ((p)->hw_pp ? (p)->hw_pp->idx - PINGPONG_0 : -1) : -1,       \
            ##__VA_ARGS__)
...
_sde_encoder_get_conn_state(struct drm_encoder *drm_enc) {
  struct msm_drm_private *priv;
  struct sde_kms *sde_kms;
  ...
  list_for_each_entry(conn_iter, connector_list,
                      head) if (conn_iter->encoder ==
                                drm_enc) return conn_iter->state;
...
int sde_encoder_in_clone_mode(struct drm_encoder *drm_enc) {
  ...
  return sde_enc && sde_enc->cur_master && sde_enc->cur_master->in_clone_mode;
}
...
int sde_encoder_helper_wait_for_irq(struct sde_encoder_phys *phys_enc,
  ...
  ret = sde_encoder_helper_wait_event_timeout(DRMID(phys_enc->parent),
                                              irq->hw_idx, wait_info);

  if (ret <= 0) {
    irq_status = sde_core_irq_read(phys_enc->sde_kms, irq->irq_idx, true);
    if (irq_status) {
      unsigned long flags;
...
int sde_encoder_helper_register_irq(struct sde_encoder_phys *phys_enc,
  ...
  if (irq->irq_idx >= 0) {
    SDE_DEBUG_PHYS(phys_enc, "skipping already registered irq %s type %d\n",
                   irq->name, irq->intr_type);
    return 0;
  }

  irq->irq_idx =
      sde_core_irq_idx_lookup(phys_enc->sde_kms, irq->intr_type, irq->hw_idx);
  if (irq->irq_idx < 0) {
    SDE_ERROR_PHYS(phys_enc, "failed to lookup IRQ index for %s type:%d\n",
                   irq->name, irq->intr_type);
    return -EINVAL;
  }

  ret =
      sde_core_irq_register_callback(phys_enc->sde_kms, irq->irq_idx, &irq->cb);
  if (ret) {
    SDE_ERROR_PHYS(phys_enc, "failed to register IRQ callback for %s\n",
                   irq->name);
    irq->irq_idx = -EINVAL;
    return ret;
  }
  ...
  if (ret) {
    SDE_ERROR_PHYS(phys_enc, "enable IRQ for intr:%s failed, irq_idx %d\n",
                   irq->name, irq->irq_idx);

    sde_core_irq_unregister_callback(phys_enc->sde_kms, irq->irq_idx, &irq->cb);
...
int sde_encoder_helper_unregister_irq(struct sde_encoder_phys *phys_enc,
  ...
  if (irq->irq_idx < 0) {
    SDE_ERROR(
        ...
        DRMID(phys_enc->parent), intr_idx, irq->hw_idx, irq->irq_idx);
    SDE_EVT32(DRMID(phys_enc->parent), intr_idx, irq->hw_idx, irq->irq_idx,
              SDE_EVTLOG_ERROR);
    return 0;
  }
  ...
  if (ret)
    SDE_EVT32(DRMID(phys_enc->parent), intr_idx, irq->hw_idx, irq->irq_idx, ret,
              SDE_EVTLOG_ERROR);
...
void sde_encoder_helper_split_config(struct sde_encoder_phys *phys_enc,
...
static int
sde_encoder_virt_atomic_check(struct drm_encoder *drm_enc,
  ...
  for (i = 0; i < sde_enc->num_phys_encs; i++) {
    ...
    if (phys && phys->ops.atomic_check)
      ret = phys->ops.atomic_check(phys, crtc_state, conn_state);
    else if (phys && phys->ops.mode_fixup)
      if (!phys->ops.mode_fixup(phys, mode, adj_mode))
        ret = -EINVAL;

    if (ret) {
      SDE_ERROR_ENC(sde_enc, "mode unsupported, phys idx %d\n", i);
      break;
    }
  }
  ...
  if (!ret) {
    ...
    old_top =
        sde_connector_get_property(conn_state, CONNECTOR_PROP_TOPOLOGY_NAME);
  ...
  if (!ret && sde_conn && drm_atomic_crtc_needs_modeset(crtc_state)) {
    ...
    ret = sde_conn->ops.get_mode_info(adj_mode, &sde_conn_state->mode_info,
                                      sde_kms->catalog->max_mixer_width,
                                      sde_conn->display);
    if (ret) {
      SDE_ERROR_ENC(sde_enc, "failed to get mode info, rc = %d\n", ret);
      return ret;
    }

    /* Reserve dynamic resources, indicating atomic_check phase */
    ret = sde_rm_reserve(&sde_kms->rm, drm_enc, crtc_state, conn_state, true);
    if (ret) {
      SDE_ERROR_ENC(sde_enc, "RM failed to reserve resources, rc = %d\n", ret);
      return ret;
    }
    ...
    ret = sde_connector_set_blob_data(conn_state->connector, conn_state,
                                      CONNECTOR_PROP_SDE_INFO);
    if (ret) {
      SDE_ERROR_ENC(sde_enc, "connector failed to update info, rc: %d\n", ret);
      return ret;
    }
  }
  ...
  if (ret) {
    SDE_ERROR_ENC(sde_enc, "connector roi check failed, rc: %d", ret);
    return ret;
  }
...
static int _sde_encoder_dsc_update_pic_dim(struct msm_display_dsc_info *dsc,
  ...
  if ((pic_width % dsc->slice_width) || (pic_height % dsc->slice_height)) {
    SDE_ERROR("pic_dim=%dx%d has to be multiple of slice=%dx%d\n", pic_width,
              pic_height, dsc->slice_width, dsc->slice_height);
    return -EINVAL;
  }
...
static bool
_sde_encoder_dsc_ich_reset_override_needed(bool pu_en,
...
static void _sde_encoder_get_connector_roi(struct sde_encoder_virt *sde_enc,
...
static int
_sde_encoder_dsc_n_lm_1_enc_1_intf(struct sde_encoder_virt *sde_enc) {
  ...
  _sde_encoder_dsc_pipe_cfg(hw_dsc, hw_pp, dsc, dsc_common_mode, ich_res, true);

  return 0;
}

static int
_sde_encoder_dsc_4_lm_4_enc_2_intf(struct sde_encoder_virt *sde_enc,
  ...
  half_panel_partial_update = hweight_long(params->affected_displays) == 1;
  ...
  _sde_encoder_dsc_pipe_cfg(hw_dsc[0], hw_pp[0], &dsc[0], dsc_common_mode,
                            ich_res, true);
  _sde_encoder_dsc_pipe_cfg(hw_dsc[1], hw_pp[1], &dsc[1], dsc_common_mode,
                            ich_res, true);
  _sde_encoder_dsc_pipe_cfg(hw_dsc[2], hw_pp[2], &dsc[2], dsc_common_mode,
                            ich_res, true);
  _sde_encoder_dsc_pipe_cfg(hw_dsc[3], hw_pp[3], &dsc[3], dsc_common_mode,
                            ich_res, true);
...
static int
_sde_encoder_dsc_2_lm_2_enc_2_intf(struct sde_encoder_virt *sde_enc,
                                   struct sde_encoder_kickoff_params *params) {
  int this_frame_slices;
  int intf_ip_w, enc_ip_w;
  int ich_res, dsc_common_mode;
  ...
  for (i = 0; i < sde_enc->num_phys_encs; i++) {
    ...
    SDE_EVT32(DRMID(&sde_enc->base), roi->w, roi->h, dsc_common_mode, i,
              active);
    _sde_encoder_dsc_pipe_cfg(hw_dsc[i], hw_pp[i], &dsc[i], dsc_common_mode,
                              ich_res, active);
  }
...
static int
_sde_encoder_dsc_2_lm_2_enc_1_intf(struct sde_encoder_virt *sde_enc,
                                   struct sde_encoder_kickoff_params *params) {
  int this_frame_slices;
  int intf_ip_w, enc_ip_w;
  int ich_res, dsc_common_mode;
  ...
  _sde_encoder_dsc_pipe_cfg(hw_dsc[0], hw_pp[0], dsc, dsc_common_mode, ich_res,
                            true);
  _sde_encoder_dsc_pipe_cfg(hw_dsc[1], hw_pp[1], dsc, dsc_common_mode, ich_res,
...
static int _sde_encoder_dsc_setup(struct sde_encoder_virt *sde_enc,
  ...
  params->num_channels = sde_rm_get_topology_num_encoders(topology);

  SDE_DEBUG_ENC(sde_enc, "topology:%d\n", topology);
  SDE_EVT32(DRMID(&sde_enc->base), topology, sde_enc->cur_conn_roi.x,
            sde_enc->cur_conn_roi.y, sde_enc->cur_conn_roi.w,
            sde_enc->cur_conn_roi.h, sde_enc->prv_conn_roi.x,
            sde_enc->prv_conn_roi.y, sde_enc->prv_conn_roi.w,
            sde_enc->prv_conn_roi.h, sde_enc->cur_master->cached_mode.hdisplay,
            sde_enc->cur_master->cached_mode.vdisplay);

  if (sde_kms_rect_is_equal(&sde_enc->cur_conn_roi, &sde_enc->prv_conn_roi))
    return ret;
  ...
  case SDE_RM_TOPOLOGY_QUADPIPE_DSCMERGE:
    ...
    break;
  default:
    SDE_ERROR_ENC(sde_enc, "No DSC support for topology %d", topology);
    return -EINVAL;
  };
...
static void _sde_encoder_update_vsync_source(struct sde_encoder_virt *sde_enc,
  ...
  if (hw_mdptop->ops.setup_vsync_source &&
    ...
    vsync_cfg.vsync_source = sde_enc->cur_master->hw_pp->caps->te_source;
    if (is_dummy)
...
static int _sde_encoder_update_rsc_client(struct drm_encoder *drm_enc,
                                          struct sde_encoder_rsc_config *config,
                                          bool enable) {
  struct sde_encoder_virt *sde_enc;
  struct drm_crtc *crtc;
  ...
  prefill_lines = config
                      ? mode_info.prefill_lines + config->inline_rotate_prefill
                      : mode_info.prefill_lines;
  ...
  if (rsc_state != SDE_RSC_IDLE_STATE && !sde_enc->rsc_state_init &&
    ...
    ret =
        sde_rsc_client_state_update(sde_enc->rsc_client, rsc_state, rsc_config,
                                    crtc->base.id, &wait_vblank_crtc_id);
  } else {
  ...
  if (wait_vblank_crtc_id)
    wait_refcount = sde_rsc_client_get_vsync_refcount(sde_enc->rsc_client);
  ...
  if (crtc->base.id != wait_vblank_crtc_id) {
    ...
    if (!primary_crtc) {
      SDE_ERROR_ENC(sde_enc, "failed to find primary crtc id %d\n",
                    wait_vblank_crtc_id);
      return -EINVAL;
    }
  ...
  for (wait_count = 0; wait_count < MAX_RSC_WAIT; wait_count++) {
    if (sde_rsc_client_is_state_update_complete(sde_enc->rsc_client))
      break;
    ...
    if (ret) {
      SDE_ERROR_ENC(sde_enc, "wait for vblank failed ret:%d\n", ret);
      /**
...
static void
_sde_encoder_resource_control_rsc_update(struct drm_encoder *drm_enc,
                                         bool enable) {
  ...
  if (enable) {
    rsc_cfg.inline_rotate_prefill = sde_crtc_get_inline_prefill(sde_enc->crtc);
...
static int _sde_encoder_resource_control_helper(struct drm_encoder *drm_enc,
  ...
  is_cmd_mode = sde_enc->disp_info.capabilities & MSM_DISPLAY_CAP_CMD_MODE;
  ...
  if (enable) {
    ...
    rc = sde_power_resource_enable(&priv->phandle, sde_kms->core_client, true);
    if (rc) {
    ...
    /* enable DSI clks */
    rc = sde_connector_clk_ctrl(sde_enc->cur_master->connector, true);
    if (rc) {
...
static void sde_encoder_input_event_handler(struct input_handle *handle,
  ...
  if (!sde_enc->crtc ||
    ...
    SDE_DEBUG_ENC(sde_enc, "invalid cached CRTC: %d or crtc index: %d\n",
                  sde_enc->crtc == NULL,
  ...
  kthread_queue_work(&event_thread->worker, &sde_enc->input_event_work);
}
...
static int sde_encoder_resource_control(struct drm_encoder *drm_enc,
  ...
  sde_enc = to_sde_encoder_virt(drm_enc);
  priv = drm_enc->dev->dev_private;
  is_vid_mode = sde_enc->disp_info.capabilities & MSM_DISPLAY_CAP_VID_MODE;

  /*
   * when idle_pc is not supported, process only KICKOFF, STOP and MODESET
   * events and return early for other events (ie wb display).
   */
  if (!sde_enc->idle_pc_enabled && (sw_event != SDE_ENC_RC_EVENT_KICKOFF &&
  ...
  case SDE_ENC_RC_EVENT_KICKOFF:
    ...
    /* BACKPORT: [Cursor] Re-enable vblank if it was gated during idle */
    if (sde_enc->vblank_enabled) {
      int i;
      for (i = 0; i < sde_enc->num_phys_encs; i++) {
        struct sde_encoder_phys *phys = sde_enc->phys_encs[i];
  ...
  case SDE_ENC_RC_EVENT_FRAME_DONE:
    ...
    if (sde_enc->cur_master && sde_enc->cur_master->ops.is_autorefresh_enabled)
      autorefresh_enabled =
          sde_enc->cur_master->ops.is_autorefresh_enabled(sde_enc->cur_master);
    ...
    /* BACKPORT: [Cursor] aggressively enter idle_pc for AOD/idle */
    if (lp == SDE_MODE_DPMS_LP2 || lp == SDE_MODE_DPMS_ON) {
    ...
    if (!autorefresh_enabled)
      kthread_mod_delayed_work(&disp_thread->worker, &sde_enc->delayed_off_work,
                               msecs_to_jiffies(idle_pc_duration));
    SDE_EVT32(DRMID(drm_enc), sw_event, sde_enc->rc_state, autorefresh_enabled,
  ...
  case SDE_ENC_RC_EVENT_PRE_STOP:
    ...
    SDE_EVT32(DRMID(drm_enc), sw_event, sde_enc->rc_state,
              SDE_ENC_RC_STATE_PRE_OFF, SDE_EVTLOG_FUNC_CASE3);
  ...
  case SDE_ENC_RC_EVENT_STOP:
    ...
    SDE_EVT32(DRMID(drm_enc), sw_event, sde_enc->rc_state, SDE_ENC_RC_STATE_OFF,
              SDE_EVTLOG_FUNC_CASE4);
  ...
  case SDE_ENC_RC_EVENT_PRE_MODESET:
    ...
    if (ret && ret != -EWOULDBLOCK) {
      ...
      SDE_EVT32(DRMID(drm_enc), sw_event, sde_enc->rc_state, ret,
                SDE_EVTLOG_ERROR);
      mutex_unlock(&sde_enc->rc_lock);
      return -EINVAL;
    }
  ...
  case SDE_ENC_RC_EVENT_POST_MODESET:
    ...
    if (sde_enc->rc_state != SDE_ENC_RC_STATE_MODESET) {
      SDE_ERROR_ENC(sde_enc, "sw_event:%d, rc:%d !MODESET state\n", sw_event,
                    sde_enc->rc_state);
      SDE_EVT32(DRMID(drm_enc), sw_event, sde_enc->rc_state, SDE_EVTLOG_ERROR);
      mutex_unlock(&sde_enc->rc_lock);
      return -EINVAL;
  ...
  case SDE_ENC_RC_EVENT_ENTER_IDLE:
    ...
    if (sde_enc->frame_busy_mask[0]) {
      SDE_ERROR_ENC(sde_enc, "sw_event:%d, rc:%d frame pending\n", sw_event,
                    sde_enc->rc_state);
      SDE_EVT32(DRMID(drm_enc), sw_event, sde_enc->rc_state, SDE_EVTLOG_ERROR);
      mutex_unlock(&sde_enc->rc_lock);
      return 0;
  ...
  case SDE_ENC_RC_EVENT_EARLY_WAKEUP:
    if (!sde_enc->crtc ||
      ...
      SDE_DEBUG_ENC(sde_enc, "invalid crtc:%d or crtc index:%d , sw_event:%u\n",
                    sde_enc->crtc == NULL,
                    sde_enc->crtc ? sde_enc->crtc->index : -EINVAL, sw_event);
      return -EINVAL;
    }
    ...
    if (sde_enc->rc_state == SDE_ENC_RC_STATE_ON) {
      if (sde_enc->cur_master &&
        ...
        autorefresh_enabled = sde_enc->cur_master->ops.is_autorefresh_enabled(
            sde_enc->cur_master);
      if (autorefresh_enabled) {
        SDE_DEBUG_ENC(
            sde_enc,
      ...
      if (!sde_crtc_frame_pending(sde_enc->crtc))
        kthread_mod_delayed_work(&disp_thread->worker,
                                 ...
                                 msecs_to_jiffies(IDLE_POWERCOLLAPSE_DURATION));
      ...
      kthread_mod_delayed_work(
          &disp_thread->worker, &sde_enc->delayed_off_work,
          msecs_to_jiffies(IDLE_POWERCOLLAPSE_IN_EARLY_WAKEUP));

      sde_enc->rc_state = SDE_ENC_RC_STATE_ON;
    }

    SDE_EVT32(DRMID(drm_enc), sw_event, sde_enc->rc_state, SDE_ENC_RC_STATE_ON,
              SDE_EVTLOG_FUNC_CASE8);
...
static void sde_encoder_virt_mode_set(struct drm_encoder *drm_enc,
  ...
  list_for_each_entry(conn_iter, connector_list,
                      head) if (conn_iter->encoder == drm_enc) conn = conn_iter;
  ...
  if (sde_conn && sde_conn_state) {
    ret = sde_conn->ops.get_mode_info(adj_mode, &sde_conn_state->mode_info,
                                      ...
                                      sde_conn->display);
    if (ret) {
      SDE_ERROR_ENC(sde_enc, "failed to get mode info from the display\n");
      return;
    }
  }
  ...
  if (msm_is_mode_seamless_dms(adj_mode)) {
    ...
    ret = sde_encoder_resource_control(drm_enc, SDE_ENC_RC_EVENT_PRE_MODESET);
    if (ret) {
  ...
  ret = sde_rm_reserve(&sde_kms->rm, drm_enc, drm_enc->crtc->state, conn->state,
                       false);
  if (ret) {
  ...
  for (i = 0; i < sde_enc->num_phys_encs; i++) {
    ...
    if (phys) {
      if (!sde_enc->hw_pp[i]) {
        SDE_ERROR_ENC(sde_enc, "invalid pingpong block for the encoder\n");
        return;
      }
  ...
  if (msm_is_mode_seamless_dms(adj_mode))
    sde_encoder_resource_control(&sde_enc->base, SDE_ENC_RC_EVENT_POST_MODESET);
}
...
static int
_sde_encoder_input_handler_register(struct input_handler *input_handler) {
  int rc = 0;
...
static int _sde_encoder_input_handler(struct sde_encoder_virt *sde_enc) {
  struct input_handler *input_handler = NULL;
  int rc = 0;

  if (sde_enc->input_handler) {
    SDE_ERROR_ENC(sde_enc, "input_handle is active. unexpected\n");
    return -EINVAL;
  }
...
static void _sde_encoder_virt_enable_helper(struct drm_encoder *drm_enc) {
  ...
  if (sde_enc->cur_master->hw_mdptop &&
    ...
    sde_enc->cur_master->hw_mdptop->ops.reset_ubwc(
        sde_enc->cur_master->hw_mdptop, sde_kms->catalog);
...
static void sde_encoder_off_work(struct kthread_work *work) {
  struct sde_encoder_virt *sde_enc =
      container_of(work, struct sde_encoder_virt, delayed_off_work.work);
  struct drm_encoder *drm_enc;
...
static void sde_encoder_virt_enable(struct drm_encoder *drm_enc) {
  ...
  if (sde_enc->input_handler && !sde_enc->input_handler_registered) {
    ret = _sde_encoder_input_handler_register(sde_enc->input_handler);
    if (ret)
      SDE_ERROR("input handler registration failed, rc = %d\n", ret);
    else
  ...
  if (!(msm_is_mode_seamless_vrr(cur_mode) ||
    ...
    kthread_init_delayed_work(&sde_enc->delayed_off_work, sde_encoder_off_work);
  ...
  for (i = 0; i < sde_enc->num_phys_encs; i++) {
    ...
    if (phys != sde_enc->cur_master) {
       ...
       * new mode.
       */
      if (msm_is_mode_seamless_dms(cur_mode) && phys->ops.restore)
        phys->ops.restore(phys);
    ...
    if (sde_enc->misr_enable &&
        (sde_enc->disp_info.capabilities & MSM_DISPLAY_CAP_VID_MODE) &&
        phys->ops.setup_misr)
      phys->ops.setup_misr(phys, true, sde_enc->misr_frame_count);
  }

  if (msm_is_mode_seamless_dms(cur_mode) && sde_enc->cur_master->ops.restore)
    sde_enc->cur_master->ops.restore(sde_enc->cur_master);
...
static void sde_encoder_vblank_callback(struct drm_encoder *drm_enc,
  ...
  /* BACKPORT: [Cursor] hardware vblank gating */
  if (sde_enc->vblank_enabled && sde_enc->crtc) {
    if (!sde_crtc_frame_pending(sde_enc->crtc) &&
        (sde_enc->rc_state == SDE_ENC_RC_STATE_IDLE ||
         sde_enc->rc_state == SDE_ENC_RC_STATE_OFF)) {
      if (phy_enc->ops.control_vblank_irq)
        phy_enc->ops.control_vblank_irq(phy_enc, false);
...
void sde_encoder_register_frame_event_callback(
    struct drm_encoder *drm_enc, void (*frame_event_cb)(void *, u32 event),
    struct drm_crtc *crtc) {
  struct sde_encoder_virt *sde_enc = to_sde_encoder_virt(drm_enc);
  unsigned long lock_flags;
  bool enable;
...
static void sde_encoder_frame_done_callback(struct drm_encoder *drm_enc,
  ...
  if (event & (SDE_ENCODER_FRAME_EVENT_DONE | SDE_ENCODER_FRAME_EVENT_ERROR |
    ...
    for (i = 0; i < sde_enc->num_phys_encs; i++) {
      if (sde_enc->phys_encs[i] == ready_phys) {
        ...
        SDE_EVT32_VERBOSE(DRMID(drm_enc), i, sde_enc->frame_busy_mask[0]);
      }
    }

    if (!sde_enc->frame_busy_mask[0]) {
      sde_encoder_resource_control(drm_enc, SDE_ENC_RC_EVENT_FRAME_DONE);
...
int sde_encoder_idle_request(struct drm_encoder *drm_enc) {
  ...
  sde_enc = to_sde_encoder_virt(drm_enc);
  sde_encoder_resource_control(&sde_enc->base, SDE_ENC_RC_EVENT_ENTER_IDLE);
...
static inline void _sde_encoder_trigger_flush(struct drm_encoder *drm_enc,
  ...
  if (phys->split_role == ENC_ROLE_SKIP) {
    SDE_DEBUG_ENC(to_sde_encoder_virt(phys->parent), "skip flush pp%d ctl%d\n",
                  phys->hw_pp->idx - PINGPONG_0, ctl->idx - CTL_0);
    return;
  }
  ...
  if (ctl->ops.get_pending_flush)
    SDE_EVT32(DRMID(drm_enc), phys->intf_idx - INTF_0, pending_kickoff_cnt,
              ctl->idx - CTL_0, ctl->ops.get_pending_flush(ctl));
  else
...
static inline void _sde_encoder_trigger_start(struct sde_encoder_phys *phys) {
  ...
  ctl = phys->hw_ctl;
  if (phys->split_role == ENC_ROLE_SKIP) {
    SDE_DEBUG_ENC(to_sde_encoder_virt(phys->parent), "skip start pp%d ctl%d\n",
                  phys->hw_pp->idx - PINGPONG_0, ctl->idx - CTL_0);
    return;
  }
...
static int _sde_encoder_wait_timeout(int32_t drm_id, int32_t hw_id,
  ...
  do {
    rc = wait_event_timeout(*(info->wq), atomic_read(info->atomic_cnt) == 0,
                            wait_time_jiffies);
    cur_ktime = ktime_get();

    SDE_EVT32(drm_id, hw_id, rc, ktime_to_ms(cur_ktime), timeout_ms,
...
int sde_encoder_helper_wait_event_timeout(int32_t drm_id, int32_t hw_id,
  ...
  if (atomic_read(info->atomic_cnt) && (!rc) &&
      (ktime_compare_safe(
           ktime_get(), ktime_add_ms(exp_ktime, FAULT_TOLERENCE_DELTA_IN_MS)) >
       0))
    rc = _sde_encoder_wait_timeout(drm_id, hw_id, FAULT_TOLERENCE_WAIT_IN_MS,
                                   info);
...
void sde_encoder_helper_hw_reset(struct sde_encoder_phys *phys_enc) {
  ...
  if (phys_enc->ops.is_master && phys_enc->ops.is_master(phys_enc) &&
    ...
    if (sde_con->ops.soft_reset) {
      ...
      if (rc) {
        SDE_ERROR_ENC(sde_enc, "connector soft reset failure\n");
...
static void _sde_encoder_kickoff_phys(struct sde_encoder_virt *sde_enc) {
  ...
  for (i = 0; i < sde_enc->num_phys_encs; i++) {
    ...
    if (phys->hw_ctl->ops.reg_dma_flush)
      phys->hw_ctl->ops.reg_dma_flush(phys->hw_ctl, is_vid_mode);
  }
  ...
  for (i = 0; i < sde_enc->num_phys_encs; i++) {
    ...
    if (!phys->ops.needs_single_flush || !phys->ops.needs_single_flush(phys))
...
static void _sde_encoder_ppsplit_swap_intf_for_right_only_update(
    struct drm_encoder *drm_enc, unsigned long *affected_displays,
    int num_active_phys) {
  struct sde_encoder_virt *sde_enc;
   ...
   */
  is_right_only =
      !test_bit(0, affected_displays) && test_bit(1, affected_displays);

  if (is_right_only && !sde_enc->intfs_swapped) {
    /* right-only update swap interfaces */
    swap(sde_enc->phys_encs[0]->intf_idx, sde_enc->phys_encs[1]->intf_idx);
  ...
  SDE_EVT32(DRMID(drm_enc), is_right_only, sde_enc->intfs_swapped,
            sde_enc->phys_encs[0]->intf_idx - INTF_0,
            sde_enc->phys_encs[1]->intf_idx - INTF_0, *affected_displays);
...
static void
_sde_encoder_update_master(struct drm_encoder *drm_enc,
                           struct sde_encoder_kickoff_params *params) {
  struct sde_encoder_virt *sde_enc;
  struct sde_encoder_phys *phys;
  ...
  SDE_EVT32_VERBOSE(DRMID(drm_enc), params->affected_displays, num_active_phys);

  /* for left/right only update, ppsplit master switches interface */
  _sde_encoder_ppsplit_swap_intf_for_right_only_update(
      drm_enc, &params->affected_displays, num_active_phys);
  ...
  for (i = 0; i < sde_enc->num_phys_encs; i++) {
    ...
    SDE_DEBUG_ENC(sde_enc, "pp %d role prv %d new %d active %d\n",
                  phys->hw_pp->idx - PINGPONG_0, prv_role, phys->split_role,
                  active);
    SDE_EVT32(DRMID(drm_enc), params->affected_displays,
              phys->hw_pp->idx - PINGPONG_0, prv_role, phys->split_role, active,
              num_active_phys);
  }
}
...
static void _sde_encoder_setup_dither(struct sde_encoder_phys *phys) {
  ...
  ret = sde_connector_get_dither_cfg(phys->connector, phys->connector->state,
                                     &dither_cfg, &len);
  if (ret)
    return;
...
static u32 _sde_encoder_calculate_linetime(struct sde_encoder_virt *sde_enc,
  ...
  SDE_DEBUG_ENC(sde_enc, "clk_rate=%lldkHz, clk_period=%d, linetime=%dns\n",
...
static int _sde_encoder_wakeup_time(struct drm_encoder *drm_enc,
  ...
  *wakeup_time = ktime_add_ns(cur_time, time_to_vsync);
  ...
  SDE_DEBUG_ENC(sde_enc,
                "cur_line=%u vtotal=%u time_to_vsync=%u, cur_time=%lld, "
                "wakeup_time=%lld\n",
                cur_line, vtotal, time_to_vsync, ktime_to_ms(cur_time),
...
static void sde_encoder_vsync_event_handler(unsigned long data) {
  ...
  kthread_queue_work(&event_thread->worker, &sde_enc->vsync_event_work);
}

static void sde_encoder_esd_trigger_work_handler(struct kthread_work *work) {
  struct sde_encoder_virt *sde_enc =
      container_of(work, struct sde_encoder_virt, esd_trigger_work);
  ...
  sde_encoder_resource_control(&sde_enc->base, SDE_ENC_RC_EVENT_KICKOFF);
}

static void sde_encoder_input_event_work_handler(struct kthread_work *work) {
  struct sde_encoder_virt *sde_enc =
      container_of(work, struct sde_encoder_virt, input_event_work);
  ...
  sde_encoder_resource_control(&sde_enc->base, SDE_ENC_RC_EVENT_EARLY_WAKEUP);
}

static void sde_encoder_vsync_event_work_handler(struct kthread_work *work) {
  struct sde_encoder_virt *sde_enc =
      container_of(work, struct sde_encoder_virt, vsync_event_work);
  bool autorefresh_enabled = false;
  int rc = 0;
  ktime_t wakeup_time;
...
int sde_encoder_poll_line_counts(struct drm_encoder *drm_enc) {
  ...
  sde_enc = to_sde_encoder_virt(drm_enc);
  if (!sde_enc->cur_master || !sde_enc->cur_master->ops.get_line_count) {
  ...
  line_count = sde_enc->cur_master->ops.get_line_count(sde_enc->cur_master);
...
int sde_encoder_prepare_for_kickoff(struct drm_encoder *drm_enc,
  ...
  if (!drm_enc || !params || !drm_enc->dev || !drm_enc->dev->dev_private) {
  ...
  if (sde_enc->cur_master && sde_enc->cur_master->ops.get_wr_line_count)
    ln_cnt1 = sde_enc->cur_master->ops.get_wr_line_count(sde_enc->cur_master);
  else
    ln_cnt1 = -EINVAL;
  ...
  for (i = 0; i < sde_enc->num_phys_encs; i++) {
    ...
    if (phys) {
      if (phys->ops.prepare_for_kickoff) {
        rc = phys->ops.prepare_for_kickoff(phys, params);
        if (rc)
          ret = rc;
      }
  ...
  if (needs_hw_reset) {
    ...
    if (sde_enc->cur_master && sde_enc->cur_master->ops.get_wr_line_count)
      ln_cnt2 = sde_enc->cur_master->ops.get_wr_line_count(sde_enc->cur_master);
    else
      ln_cnt2 = -EINVAL;

    SDE_EVT32(DRMID(drm_enc), ln_cnt1, ln_cnt2, SDE_EVTLOG_FUNC_CASE1);
    for (i = 0; i < sde_enc->num_phys_encs; i++) {
      phys = sde_enc->phys_encs[i];
    ...
    for (i = 0; i < sde_enc->num_phys_encs; i++) {
      ...
      if (phys) {
        ...
        mode_is_yuv =
            ((mode->private_flags & MSM_MODE_FLAG_COLOR_FORMAT_YCBCR420) ||
             (mode->private_flags & MSM_MODE_FLAG_COLOR_FORMAT_YCBCR422));
      }
      /**
      ...
      if (phys && mode_is_yuv) {
        if (phys->enc_cdm_csc != conn_csc) {
          ...
          if (rc)
            SDE_ERROR_ENC(sde_enc, "CSC setup failed rc %d\n", rc);
          SDE_DEBUG_ENC(sde_enc, "updating CSC %d to %d\n", phys->enc_cdm_csc,
                        conn_csc);
...
int sde_encoder_helper_reset_mixers(struct sde_encoder_phys *phys_enc,
  ...
  while (sde_rm_get_hw(&phys_enc->sde_kms->rm, &lm_iter)) {
    ...
    if (phys_enc->hw_ctl->ops.get_bitmask_mixer &&
      ...
      phys_enc->hw_ctl->ops.update_pending_flush(
          phys_enc->hw_ctl, phys_enc->hw_ctl->ops.get_bitmask_mixer(
    ...
    if (phys_enc->hw_ctl->ops.setup_blendstage)
      phys_enc->hw_ctl->ops.setup_blendstage(phys_enc->hw_ctl, hw_lm->idx,
...
static int _sde_encoder_status_show(struct seq_file *s, void *data) {
  ...
  for (i = 0; i < sde_enc->num_phys_encs; i++) {
    ...
    seq_printf(s, "intf:%d    vsync:%8d     underrun:%8d    ",
               phys->intf_idx - INTF_0, atomic_read(&phys->vsync_cnt),
...
static ssize_t _sde_encoder_misr_read(struct file *file, char __user *user_buff,
                                      size_t count, loff_t *ppos) {
  struct sde_encoder_virt *sde_enc;
  int i = 0, len = 0;
  ...
  if (!sde_enc->misr_enable) {
    ...
    goto buff_check;
  } else if (sde_enc->disp_info.capabilities & ~MSM_DISPLAY_CAP_VID_MODE) {
    len += snprintf(buf + len, MISR_BUFF_SIZE - len, "unsupported\n");
    goto buff_check;
  }
  ...
  for (i = 0; i < sde_enc->num_phys_encs; i++) {
    ...
    len += snprintf(buf + len, MISR_BUFF_SIZE - len, "Intf idx:%d\n",
...
static int _sde_encoder_init_debugfs(struct drm_encoder *drm_enc) {
  ...
  sde_enc->debugfs_root =
      debugfs_create_dir(name, drm_enc->dev->primary->debugfs_root);
  if (!sde_enc->debugfs_root)
    return -ENOMEM;

  /* don't error check these */
  debugfs_create_file("status", 0600, sde_enc->debugfs_root, sde_enc,
                      &debugfs_status_fops);

  debugfs_create_file("misr_data", 0600, sde_enc->debugfs_root, sde_enc,
                      &debugfs_misr_fops);

  for (i = 0; i < sde_enc->num_phys_encs; i++)
    if (sde_enc->phys_encs[i] && sde_enc->phys_encs[i]->ops.late_register)
      sde_enc->phys_encs[i]->ops.late_register(sde_enc->phys_encs[i],
                                               sde_enc->debugfs_root);
...
#else
static int _sde_encoder_init_debugfs(struct drm_encoder *drm_enc) { return 0; }
...
static int
sde_encoder_virt_add_phys_encs(u32 display_caps,
                               struct sde_encoder_virt *sde_enc,
...
static int
sde_encoder_virt_add_phys_enc_wb(struct sde_encoder_virt *sde_enc,
                                 struct sde_enc_phys_init_params *params) {
  struct sde_encoder_phys *enc = NULL;
...
static int sde_encoder_setup_display(struct sde_encoder_virt *sde_enc,
  ...
  for (i = 0; i < disp_info->num_of_h_tiles && !ret; i++) {
    ...
    if (intf_type == INTF_WB) {
      ...
      phys_params.wb_idx =
          sde_encoder_get_wb(sde_kms->catalog, intf_type, controller_id);
      if (phys_params.wb_idx == WB_MAX) {
        SDE_ERROR_ENC(sde_enc, "could not get wb: type %d, id %d\n", intf_type,
                      controller_id);
        ret = -EINVAL;
      }
    } else {
      ...
      phys_params.intf_idx =
          sde_encoder_get_intf(sde_kms->catalog, intf_type, controller_id);
    ...
    if (!ret) {
      if (intf_type == INTF_WB)
        ret = sde_encoder_virt_add_phys_enc_wb(sde_enc, &phys_params);
      else
        ret = sde_encoder_virt_add_phys_encs(disp_info->capabilities, sde_enc,
                                             &phys_params);
      if (ret)
...
struct drm_encoder *sde_encoder_init(struct drm_device *dev,
  ...
  ret = sde_encoder_setup_display(sde_enc, sde_kms, disp_info, &drm_enc_mode);
  if (ret)
    goto fail;
  ...
  if ((disp_info->intf_type == DRM_MODE_CONNECTOR_DSI) && disp_info->is_primary)
    setup_timer(&sde_enc->vsync_event_timer, sde_encoder_vsync_event_handler,
                (unsigned long)sde_enc);

  snprintf(name, SDE_NAME_SIZE, "rsc_enc%u", drm_enc->base.id);
  sde_enc->rsc_client =
      sde_rsc_client_create(SDE_RSC_INDEX, name, disp_info->is_primary);
...
int sde_encoder_update_caps_for_cont_splash(struct drm_encoder *encoder) {
  ...
  SDE_DEBUG_ENC(sde_enc, "num of connectors: %d\n", priv->num_connectors);

  for (i = 0; i < priv->num_connectors; i++) {
    SDE_DEBUG_ENC(sde_enc, "connector id: %d\n", priv->connectors[i]->base.id);
    sde_conn = to_sde_connector(priv->connectors[i]);
    if (!sde_conn->encoder) {
      SDE_DEBUG_ENC(sde_enc, "encoder not attached to connector\n");
      continue;
    }
    if (sde_conn->encoder->base.id == encoder->base.id) {
  ...
  ret = sde_conn->ops.get_mode_info(
      &encoder->crtc->state->adjusted_mode, &sde_conn_state->mode_info,
      sde_kms->catalog->max_mixer_width, sde_conn->display);
  if (ret) {
    SDE_ERROR_ENC(sde_enc, "conn: ->get_mode_info failed. ret=%d\n", ret);
    return ret;
  }

  ret = sde_rm_reserve(&sde_kms->rm, encoder, encoder->crtc->state, conn->state,
                       false);
  if (ret) {
    SDE_ERROR_ENC(sde_enc, "failed to reserve hw resources, %d\n", ret);
    return ret;
  ...
  if (sde_conn->encoder) {
    ...
    SDE_DEBUG_ENC(sde_enc, "configured cstate->best_encoder to ID = %d\n",
  ...
  if (encoder->bridge) {
    ...
    if (encoder->bridge->funcs) {
      ...
      encoder->bridge->funcs->mode_set(encoder->bridge, drm_mode, drm_mode);
    }
  } else {
  ...
  for (i = 0; i < sde_enc->num_phys_encs; i++) {
    ...
    if (!phys) {
      SDE_ERROR_ENC(sde_enc, "phys encoders not initialized\n");
      return -EINVAL;
    }

    /* update connector for master and slave phys encoders */
    phys->connector = conn;
    phys->cont_splash_single_flush = sde_kms->splash_data.single_flush_en;
...
int sde_encoder_display_failure_notification(struct drm_encoder *enc) {
  ...
  kthread_queue_work(&event_thread->worker, &sde_enc->esd_trigger_work);
...
void sde_encoder_phys_setup_cdm(struct sde_encoder_phys *phys_enc,
                                ...
                                u32 output_type, struct sde_rect *roi) {
  ...
  if ((output_type == CDM_CDWN_OUTPUT_WB) && !SDE_FORMAT_IS_YUV(format)) {
  ...
  cdm_cfg->output_bit_depth =
      SDE_FORMAT_IS_DX(format) ? CDM_CDWN_OUTPUT_10BIT : CDM_CDWN_OUTPUT_8BIT;
  ...
  SDE_DEBUG_ENC(sde_enc, "[cdm_enable:%d,%d,%X,%d,%d,%d,%d]\n",
                cdm_cfg->output_width, cdm_cfg->output_height,
                cdm_cfg->output_fmt->base.pixel_format, cdm_cfg->output_type,
                cdm_cfg->output_bit_depth, cdm_cfg->h_cdwn_type,
                cdm_cfg->v_cdwn_type);
  ...
  if (output_type == CDM_CDWN_OUTPUT_HDMI) {
    if (SDE_FORMAT_IS_YUV(format)) {
      ...
      else if (connector && sde_connector_mode_needs_full_range(connector))
        csc_type = SDE_CSC_RGB2YUV_709FR;
      else
        csc_type = SDE_CSC_RGB2YUV_709L;
    } else if (connector && sde_connector_mode_is_cea_mode(connector)) {
  ...
  if (hw_cdm && hw_cdm->ops.setup_csc_data) {
    ret = hw_cdm->ops.setup_csc_data(hw_cdm, &sde_csc_10bit_convert[csc_type]);
    if (ret < 0) {
