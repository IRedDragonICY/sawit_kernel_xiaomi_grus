...
struct sde_crtc_custom_events {
  u32 event;
  int (*func)(struct drm_crtc *crtc, bool en, struct sde_irq_callback *irq);
};

static int sde_crtc_power_interrupt_handler(struct drm_crtc *crtc_drm, bool en,
                                            struct sde_irq_callback *ad_irq);
static int sde_crtc_idle_interrupt_handler(struct drm_crtc *crtc_drm, bool en,
                                           struct sde_irq_callback *idle_irq);
...
static inline int _sde_crtc_power_enable(struct sde_crtc *sde_crtc,
                                         bool enable) {
...
static void sde_crtc_calc_fps(struct sde_crtc *sde_crtc) {
  ...
  if (diff_us >= CRTC_TIME_PERIOD_CALC_FPS_US) {
    ...
    SDE_DEBUG(" FPS for crtc%d is %d.%d\n", sde_crtc->base.base.id,
              (unsigned int)fps / 10, (unsigned int)fps % 10);
...
static void _sde_crtc_rp_reclaim(struct sde_crtc_respool *rp, bool force) {
  ...
  list_for_each_entry_safe(res, next, &rp->res_list, list) {
    ...
    SDE_DEBUG("crtc%d.%u reclaim res:0x%x/0x%llx/%pK/%d\n", crtc->base.id,
              rp->sequence_id, res->type, res->tag, res->val,
...
static void _sde_crtc_rp_duplicate(struct sde_crtc_respool *rp,
  ...
  list_for_each_entry(res, &rp->res_list, list) {
    ...
    SDE_DEBUG("crtc%d.%u dup res:0x%x/0x%llx/%pK/%d\n", crtc->base.id,
              dup_rp->sequence_id, dup_res->type, dup_res->tag, dup_res->val,
   ...
   */
    ...
    list_for_each_entry(old_rp, rp->rp_head, rp_list) {
      ...
      list_for_each_entry(res, &old_rp->res_list, list) {
        if (res->type != type)
          continue;
        SDE_DEBUG("crtc%d.%u found res:0x%x//%pK/ in crtc%d.%d\n",
                  crtc->base.id, rp->sequence_id, res->type, res->val,
                  crtc->base.id, old_rp->sequence_id);
        SDE_EVT32_VERBOSE(crtc->base.id, rp->sequence_id, res->type, res->val,
                          crtc->base.id, old_rp->sequence_id);
        if (res->ops.get)
...
static void _sde_crtc_rp_put(struct sde_crtc_respool *rp, u32 type, u64 tag) {
  ...
  list_for_each_entry_safe(res, next, &rp->res_list, list) {
    ...
    if (res->flags & SDE_CRTC_RES_FLAG_FREE)
      SDE_ERROR("crtc%d.%u already free res:0x%x/0x%llx/%pK/%d\n",
                crtc->base.id, rp->sequence_id, res->type, res->tag, res->val,
                atomic_read(&res->refcount));
...
int sde_crtc_res_add(struct drm_crtc_state *state, u32 type, u64 tag, void *val,
                     struct sde_crtc_res_ops *ops) {
...
static int _sde_debugfs_fps_status(struct inode *inode, struct file *file) {
  return single_open(file, _sde_debugfs_fps_status_show, inode->i_private);
}
...
static struct attribute *sde_crtc_dev_attrs[] = {&dev_attr_vsync_event.attr,
                                                 NULL};
...
static void _sde_crtc_setup_blend_cfg(struct sde_crtc_mixer *mixer,
  ...
  case SDE_DRM_BLEND_OP_PREMULTIPLIED:
    if (format->alpha_enable) {
      blend_op = SDE_BLEND_FG_ALPHA_FG_CONST | SDE_BLEND_BG_ALPHA_FG_PIXEL;
      if (fg_alpha != 0xff) {
        bg_alpha = fg_alpha;
        blend_op |= SDE_BLEND_BG_MOD_ALPHA | SDE_BLEND_BG_INV_MOD_ALPHA;
      } else {
  ...
  case SDE_DRM_BLEND_OP_COVERAGE:
    if (format->alpha_enable) {
      blend_op = SDE_BLEND_FG_ALPHA_FG_PIXEL | SDE_BLEND_BG_ALPHA_FG_PIXEL;
      if (fg_alpha != 0xff) {
        bg_alpha = fg_alpha;
        blend_op |= SDE_BLEND_FG_MOD_ALPHA | SDE_BLEND_FG_INV_MOD_ALPHA |
                    SDE_BLEND_BG_MOD_ALPHA | SDE_BLEND_BG_INV_MOD_ALPHA;
      } else {
        blend_op |= SDE_BLEND_BG_INV_ALPHA;
      }
    }
  ...
  SDE_DEBUG("format: %4.4s, alpha_enable %u fg alpha:0x%x bg alpha:0x%x "
            "blend_op:0x%x\n",
            (char *)&format->base.pixel_format, format->alpha_enable, fg_alpha,
            bg_alpha, blend_op);
}
...
static void _sde_crtc_setup_dim_layer_cfg(struct drm_crtc *crtc,
  ...
  for (i = 0; i < sde_crtc->num_mixers; i++) {
    ...
    if (sde_kms_rect_is_null(&split_dim_layer.rect)) {
      ...
      split_dim_layer.rect.x = split_dim_layer.rect.x - cstate->lm_roi[i].x;
      split_dim_layer.rect.y = split_dim_layer.rect.y - cstate->lm_roi[i].y;
    }

    SDE_EVT32_VERBOSE(DRMID(crtc), cstate->lm_roi[i].x, cstate->lm_roi[i].y,
                      cstate->lm_roi[i].w, cstate->lm_roi[i].h,
                      dim_layer->rect.x, dim_layer->rect.y, dim_layer->rect.w,
                      dim_layer->rect.h, split_dim_layer.rect.x,
                      split_dim_layer.rect.y, split_dim_layer.rect.w,
                      split_dim_layer.rect.h);

    SDE_DEBUG("split_dim_layer - LM:%d, rect:{%d,%d,%d,%d}}\n", i,
              split_dim_layer.rect.x, split_dim_layer.rect.y,
...
static int _sde_crtc_set_roi_v1(struct drm_crtc_state *state,
  ...
  for (i = 0; i < roi_v1.num_rects; ++i) {
    ...
    SDE_DEBUG("crtc%d: roi%d: roi (%d,%d) (%d,%d)\n", DRMID(crtc), i,
              cstate->user_roi_list.roi[i].x1, cstate->user_roi_list.roi[i].y1,
              cstate->user_roi_list.roi[i].x2, cstate->user_roi_list.roi[i].y2);
    SDE_EVT32_VERBOSE(DRMID(crtc), cstate->user_roi_list.roi[i].x1,
...
static int _sde_crtc_set_crtc_roi(struct drm_crtc *crtc,
  ...
  for_each_connector_in_state(state->state, conn, conn_state, i) {
    ...
    SDE_EVT32_VERBOSE(DRMID(crtc), DRMID(conn), conn_roi.x, conn_roi.y,
  ...
  SDE_DEBUG("%s: crtc roi (%d,%d,%d,%d)\n", sde_crtc->name, crtc_roi->x,
            crtc_roi->y, crtc_roi->w, crtc_roi->h);
...
static int _sde_crtc_check_autorefresh(struct drm_crtc *crtc,
  ...
  for_each_connector_in_state(state->state, conn, conn_state, i) {
    ...
    autorefresh =
        sde_connector_get_property(conn_state, CONNECTOR_PROP_AUTOREFRESH);
    if (autorefresh) {
      SDE_ERROR("%s: autorefresh & partial crtc roi incompatible %llu\n",
...
static int
_sde_crtc_check_rois_centered_and_symmetric(struct drm_crtc *crtc,
                                            struct drm_crtc_state *state) {
  struct sde_crtc *sde_crtc;
  struct sde_crtc_state *crtc_state;
  ...
  if (crtc_state->is_ppsplit) {
    ...
    if (roi[0]->x + roi[0]->w / 2 != panel_split_width) {
      SDE_ERROR("%s: roi x %d w %d split %d\n", sde_crtc->name, roi[0]->x,
                roi[0]->w, panel_split_width);
      return -EINVAL;
    }
  ...
  if (roi[0]->x + roi[0]->w != roi[1]->x || roi[0]->w != roi[1]->w) {
    SDE_ERROR(
        ...
        sde_crtc->name, roi[0]->x, roi[0]->w, roi[1]->x, roi[1]->w);
    return -EINVAL;
  }
...
static int
_sde_crtc_check_planes_within_crtc_roi(struct drm_crtc *crtc,
                                       struct drm_crtc_state *state) {
  struct sde_crtc *sde_crtc;
  struct sde_crtc_state *crtc_state;
  const struct sde_rect *crtc_roi;
  ...
  drm_atomic_crtc_state_for_each_plane_state(plane, pstate, state) {
    ...
    if (!sde_kms_rect_is_equal(&plane_roi, &intersection)) {
      SDE_ERROR(
          ...
          sde_crtc->name, plane->base.id, plane_roi.x, plane_roi.y, plane_roi.w,
          plane_roi.h, crtc_roi->x, crtc_roi->y, crtc_roi->w, crtc_roi->h);
      return -E2BIG;
    }
  }
...
static int _sde_crtc_check_rois(struct drm_crtc *crtc,
  ...
  for (i = 0; i < ARRAY_SIZE(sde_crtc_state->connectors); i++) {
    ...
    if (sde_crtc_state->user_roi_list.num_rects > mode_info.roi_caps.num_roi) {
...
static void _sde_crtc_program_lm_output_roi(struct drm_crtc *crtc) {
  ...
  for (lm_idx = 0; lm_idx < sde_crtc->num_mixers; lm_idx++) {
    ...
    SDE_EVT32(DRMID(crtc_state->base.crtc), lm_idx, lm_roi->x, lm_roi->y,
              lm_roi->w, lm_roi->h);
...
static void _sde_crtc_blend_setup_mixer(struct drm_crtc *crtc,
  ...
  drm_atomic_crtc_for_each_plane(plane, crtc) {
    ...
    if (old_state &&
        drm_atomic_get_existing_plane_state(old_state->state, plane))
      sde_crtc->sbuf_flush_mask_delta |= flush_sbuf;
    sde_crtc->sbuf_flush_mask_all |= flush_sbuf;

    SDE_DEBUG("crtc %d stage:%d - plane %d sspp %d fb %d\n", crtc->base.id,
              pstate->stage, plane->base.id, sde_plane_pipe(plane) - SSPP_VIG0,
    ...
    SDE_EVT32(DRMID(crtc), DRMID(plane), state->fb ? state->fb->base.id : -1,
              state->src_x >> 16, state->src_y >> 16, state->src_w >> 16,
              state->src_h >> 16, state->crtc_x, state->crtc_y, state->crtc_w,
              state->crtc_h, flush_sbuf != 0);

    stage_idx = zpos_cnt[pstate->stage]++;
    stage_cfg->stage[pstate->stage][stage_idx] = sde_plane_pipe(plane);
    ...
    for (lm_idx = 0; lm_idx < sde_crtc->num_mixers; lm_idx++) {
      ...
      if (bg_alpha_enable && !format->alpha_enable)
        ...
        mixer[lm_idx].mixer_op_mode |= 1 << pstate->stage;
    }
  }
...
static void
_sde_crtc_swap_mixers_for_right_partial_update(struct drm_crtc *crtc) {
  struct sde_crtc *sde_crtc;
  struct sde_crtc_state *cstate;
  ...
  drm_for_each_encoder(drm_enc, crtc->dev) {
    if (drm_enc->crtc == crtc && sde_encoder_is_dsc_merge(drm_enc)) {
...
static void _sde_crtc_blend_setup(struct drm_crtc *crtc,
  ...
  for (i = 0; i < sde_crtc->num_mixers; i++) {
    ...
    if (mixer[i].hw_ctl->ops.clear_all_blendstages)
      mixer[i].hw_ctl->ops.clear_all_blendstages(mixer[i].hw_ctl);
  ...
  for (i = 0; i < sde_crtc->num_mixers; i++) {
    ...
    if (sde_kms_rect_is_null(lm_roi)) {
      SDE_DEBUG("%s: lm%d leave ctl%d mask 0 since null roi\n", sde_crtc->name,
                lm->idx - LM_0, ctl->idx - CTL_0);
      continue;
    }
    ...
    mixer[i].flush_mask |= ctl->ops.get_bitmask_mixer(ctl, mixer[i].hw_lm->idx);

    /* stage config flush mask */
    ctl->ops.update_pending_flush(ctl, mixer[i].flush_mask);

    SDE_DEBUG("lm %d, op_mode 0x%X, ctl %d, flush mask 0x%x\n",
              mixer[i].hw_lm->idx - LM_0, mixer[i].mixer_op_mode,
              ctl->idx - CTL_0, mixer[i].flush_mask);

    ctl->ops.setup_blendstage(ctl, mixer[i].hw_lm->idx,
                              mixer[i].hw_lm->cfg.flags, &sde_crtc->stage_cfg);
  }
...
static int _sde_crtc_find_plane_fb_modes(struct drm_crtc_state *state,
                                         uint32_t *fb_ns, uint32_t *fb_sec,
                                         uint32_t *fb_sec_dir) {
  struct drm_plane *plane;
  const struct drm_plane_state *pstate;
  ...
  *fb_sec_dir = 0;
  drm_atomic_crtc_state_for_each_plane_state(plane, pstate, state) {
    ...
    mode = sde_plane_get_property(sde_pstate, PLANE_PROP_FB_TRANSLATION_MODE);
    switch (mode) {
    ...
    case SDE_DRM_FB_SEC_DIR_TRANS:
      ...
      break;
    default:
      SDE_ERROR("Error: Plane[%d], fb_trans_mode:%d", plane->base.id, mode);
      return -EINVAL;
    }
  }
  return 0;
...
int sde_crtc_get_secure_transition_ops(struct drm_crtc *crtc,
  ...
  SDE_DEBUG("crtc%d, secure_level%d old_valid_fb%d\n", crtc->base.id,
            secure_level, old_valid_fb);

  SDE_EVT32_VERBOSE(DRMID(crtc), secure_level, smmu_state->state, old_valid_fb,
  ...
  drm_for_each_encoder(encoder, crtc->dev) {
    ...
    post_commit |= sde_encoder_check_mode(encoder, MSM_DISPLAY_CAP_VID_MODE);
  }
  ...
  drm_atomic_crtc_for_each_plane(plane, crtc) {
    ...
    translation_mode = sde_plane_get_property(to_sde_plane_state(plane->state),
  ...
  case SDE_DRM_FB_SEC_DIR_TRANS:
    ...
    if ((smmu_state->state == ATTACHED) && (secure_level == SDE_DRM_SEC_ONLY)) {
      ...
      if (catalog->sui_misr_supported && sde_crtc->enable_sui_enhancement)
  ...
  case SDE_DRM_FB_NON_SEC:
    if ((smmu_state->state == DETACHED_SEC) ||
      ...
      smmu_state->transition_type = post_commit ? POST_COMMIT : PRE_COMMIT;
      ops |= SDE_KMS_OPS_SECURE_STATE_CHANGE;
      if (old_valid_fb)
  ...
  if (ops)
    SDE_EVT32(DRMID(crtc), secure_level, translation_mode, smmu_state->state,
              smmu_state->transition_type, ops, old_valid_fb,
...
static int _sde_validate_hw_resources(struct sde_crtc *sde_crtc) {
  ...
  if (!sde_crtc->num_mixers || sde_crtc->num_mixers > MAX_MIXERS_PER_CRTC) {
    SDE_ERROR("%s: invalid number mixers: %d\n", sde_crtc->name,
              sde_crtc->num_mixers);
    SDE_EVT32(DRMID(&sde_crtc->base), sde_crtc->num_mixers, SDE_EVTLOG_ERROR);
    return -EINVAL;
  }
  ...
  for (i = 0; i < sde_crtc->num_mixers; i++) {
    if (!sde_crtc->mixers[i].hw_lm || !sde_crtc->mixers[i].hw_ctl ||
      ...
      SDE_EVT32(DRMID(&sde_crtc->base), sde_crtc->num_mixers, i,
                sde_crtc->mixers[i].hw_lm, sde_crtc->mixers[i].hw_ctl,
  ...
  if (!ds_dirty) {
    ...
    for (i = 0; i < count; i++) {
      ...
      if ((i == count - 1) && hw_ds->ops.setup_opmode) {
        op_mode |= (cstate->num_ds_enabled == CRTC_DUAL_MIXERS)
                       ? SDE_DS_OP_MODE_DUAL
                       : 0;
      ...
      if ((cfg->flags & SDE_DRM_DESTSCALER_SCALE_UPDATE) ||
        ...
        if (hw_ds->ops.setup_scaler)
          hw_ds->ops.setup_scaler(hw_ds, &cfg->scl3_cfg, &cstate->scl3_lut_cfg);
      }
      ...
      if (hw_ctl->ops.get_bitmask_mixer) {
        flush_mask = hw_ctl->ops.get_bitmask_mixer(hw_ctl, hw_lm->idx);
        SDE_DEBUG("Set lm[%d] flush = %d", hw_lm->idx, flush_mask);
...
void sde_crtc_prepare_commit(struct drm_crtc *crtc,
  ...
  drm_for_each_connector(
      conn, crtc->dev) if (conn->state && conn->state->crtc == crtc &&
...
static void sde_crtc_vblank_cb(void *data) {
  ...
  /* BACKPORT: [Cursor] fast-path drop to reduce overhead if no pending
   * frame/event */
  if (atomic_read(&sde_crtc->frame_pending) == 0 &&
...
static void _sde_crtc_retire_event(struct drm_connector *connector, ktime_t ts,
                                   bool is_error) {
  if (!connector) {
  ...
  if (!in_clone_mode && (fevent->event & (SDE_ENCODER_FRAME_EVENT_ERROR |
    ...
    if (atomic_read(&sde_crtc->frame_pending) < 1) {
      ...
      SDE_ERROR("crtc%d ts:%lld invalid frame_pending:%d\n", crtc->base.id,
                ktime_to_ns(fevent->ts), atomic_read(&sde_crtc->frame_pending));
      SDE_EVT32(DRMID(crtc), fevent->event, SDE_EVTLOG_FUNC_CASE1);
    } else if (atomic_dec_return(&sde_crtc->frame_pending) == 0) {
      /* release bandwidth and other resources */
      SDE_DEBUG("crtc%d ts:%lld last pending\n", crtc->base.id,
                ktime_to_ns(fevent->ts));
      SDE_EVT32(DRMID(crtc), fevent->event, SDE_EVTLOG_FUNC_CASE2);
      sde_core_perf_crtc_release_bw(crtc);
    } else {
      SDE_EVT32_VERBOSE(DRMID(crtc), fevent->event, SDE_EVTLOG_FUNC_CASE3);
    }
  }
...
static void _sde_crtc_set_dim_layer_v1(struct sde_crtc_state *cstate,
  ...
  for (i = 0; i < count; i++) {
    ...
    SDE_DEBUG(" rect:{%d,%d,%d,%d}, color:{%d,%d,%d,%d}\n", dim_layer[i].rect.x,
              dim_layer[i].rect.y, dim_layer[i].rect.w, dim_layer[i].rect.h,
              dim_layer[i].color_fill.color_0, dim_layer[i].color_fill.color_1,
              dim_layer[i].color_fill.color_2, dim_layer[i].color_fill.color_3);
  }
}
...
static int _sde_crtc_set_dest_scaler(struct sde_crtc *sde_crtc,
  ...
  for (i = 0; i < count; i++) {
    ...
    if (ds_cfg_usr->scaler_cfg) {
      scaler_v2_usr = (void __user *)((uintptr_t)ds_cfg_usr->scaler_cfg);

      if (copy_from_user(&scaler_v2, scaler_v2_usr, sizeof(scaler_v2))) {
    ...
    SDE_DEBUG("en(%d)dir(%d)de(%d) src(%dx%d) dst(%dx%d)\n", scaler_v2.enable,
              scaler_v2.dir_en, scaler_v2.de.enable, scaler_v2.src_width[0],
              scaler_v2.src_height[0], scaler_v2.dst_width,
              scaler_v2.dst_height);
    SDE_EVT32_VERBOSE(DRMID(&sde_crtc->base), scaler_v2.enable,
                      scaler_v2.dir_en, scaler_v2.de.enable,
...
static int _sde_crtc_check_dest_scaler_data(struct drm_crtc *crtc,
  ...
  if (cstate->num_ds > kms->catalog->ds_count ||
    ...
    SDE_ERROR("crtc%d: num_ds(%d), hw_ds_cnt(%d) flags(%d)\n", crtc->base.id,
              cstate->num_ds, kms->catalog->ds_count, cstate->ds_cfg[0].flags);
    ret = -EINVAL;
    goto err;
  }
  ...
  for (i = 0; i < cstate->num_ds; i++) {
     ...
     */
    if (lm_idx >= sde_crtc->num_mixers ||
        (i != lm_idx && !(cfg->flags & SDE_DRM_DESTSCALER_PU_ENABLE))) {
      SDE_ERROR("crtc%d: ds_cfg id(%d):idx(%d), flags(%d)\n", crtc->base.id, i,
                lm_idx, cfg->flags);
      SDE_EVT32(DRMID(crtc), i, lm_idx, cfg->flags, SDE_EVTLOG_ERROR);
      ret = -EINVAL;
      goto err;
    }
    ...
    if (cfg->lm_width > hdisplay || cfg->lm_height > mode->vdisplay ||
      ...
      SDE_ERROR("crtc%d: lm size[%d,%d] display [%d,%d]\n", crtc->base.id,
                cfg->lm_width, cfg->lm_height, hdisplay, mode->vdisplay);
      SDE_EVT32(DRMID(crtc), cfg->lm_width, cfg->lm_height, hdisplay,
                mode->vdisplay, SDE_EVTLOG_ERROR);
      ret = -E2BIG;
      goto err;
    }
    ...
    if (!prev_lm_width && !prev_lm_height) {
      ...
      if (cfg->lm_width != prev_lm_width || cfg->lm_height != prev_lm_height) {
        SDE_ERROR("crtc%d:lm left[%d,%d]right[%d %d]\n", crtc->base.id,
                  cfg->lm_width, cfg->lm_height, prev_lm_width, prev_lm_height);
        SDE_EVT32(DRMID(crtc), cfg->lm_width, cfg->lm_height, prev_lm_width,
    ...
    if (cfg->flags & SDE_DRM_DESTSCALER_SCALE_UPDATE ||
      ...
      if (cfg->scl3_cfg.src_width[0] > max_in_width ||
          ...
          !cfg->scl3_cfg.src_width[0] || !cfg->scl3_cfg.dst_width ||
        ...
        SDE_ERROR("src_w(%d) dst(%dx%d) display(%dx%d)",
                  cfg->scl3_cfg.src_width[0], cfg->scl3_cfg.dst_width,
                  cfg->scl3_cfg.dst_height, hdisplay, mode->vdisplay);
        SDE_ERROR("num_mixers(%d) flags(%d) ds-%d:\n", sde_crtc->num_mixers,
                  cfg->flags, hw_ds->idx - DS_0);
        SDE_ERROR("scale_en = %d, DE_en =%d\n", cfg->scl3_cfg.enable,
                  cfg->scl3_cfg.de.enable);

        SDE_EVT32(DRMID(crtc), cfg->scl3_cfg.enable, cfg->scl3_cfg.de.enable,
                  cfg->flags, max_in_width, max_out_width,
                  cfg->scl3_cfg.src_width[0], cfg->scl3_cfg.dst_width,
                  cfg->scl3_cfg.dst_height, hdisplay, mode->vdisplay,
                  sde_crtc->num_mixers, SDE_EVTLOG_ERROR);
...
static void _sde_crtc_setup_mixer_for_encoder(struct drm_crtc *crtc,
  ...
  for (i = sde_crtc->num_mixers; i < ARRAY_SIZE(sde_crtc->mixers); i++) {
    ...
    mixer->hw_lm->cfg.right_mixer = (sde_crtc->num_mixers & 1) ? true : false;
...
static void _sde_crtc_setup_is_ppsplit(struct drm_crtc_state *state) {
  ...
  for (i = 0; i < cstate->num_connectors; i++) {
    ...
    if (sde_connector_get_topology_name(conn) == SDE_RM_TOPOLOGY_PPSPLIT)
...
static void _sde_crtc_setup_lm_bounds(struct drm_crtc *crtc,
  ...
  for (i = 0; i < sde_crtc->num_mixers; i++) {
    ...
    SDE_EVT32_VERBOSE(DRMID(crtc), i, cstate->lm_bounds[i].x,
                      cstate->lm_bounds[i].y, cstate->lm_bounds[i].w,
                      cstate->lm_bounds[i].h);
    SDE_DEBUG("%s: lm%d bnd&roi (%d,%d,%d,%d)\n", sde_crtc->name, i,
              cstate->lm_roi[i].x, cstate->lm_roi[i].y, cstate->lm_roi[i].w,
              cstate->lm_roi[i].h);
  }
...
static void sde_crtc_atomic_flush(struct drm_crtc *crtc,
   ...
   */
  drm_atomic_crtc_for_each_plane(plane, crtc) sde_plane_restore(plane);
...
static int _sde_crtc_commit_kickoff_rot(struct drm_crtc *crtc,
  ...
  if ((sde_crtc->sbuf_op_mode_old != cstate->sbuf_cfg.rot_op_mode) &&
    ...
    flush_mask |= sde_crtc->sbuf_flush_mask_all | sde_crtc->sbuf_flush_mask_old;
  ...
  if (master_ctl && master_ctl->ops.update_pending_flush) {
    ...
    SDE_EVT32(DRMID(crtc), master_ctl->idx - CTL_0,
              sde_crtc->sbuf_flush_mask_all, sde_crtc->sbuf_flush_mask_delta);
  }
...
static void _sde_crtc_remove_pipe_flush(struct sde_crtc *sde_crtc) {
  ...
  for (i = 0; i < n; i++) {
    ctl = mixer[i].hw_ctl;
    if (!ctl || !ctl->ops.get_pending_flush || !ctl->ops.clear_pending_flush ||
...
static int _sde_crtc_reset_hw(struct drm_crtc *crtc,
  ...
  SDE_EVT32(DRMID(crtc), old_rot_op_mode, dump_status, SDE_EVTLOG_FUNC_ENTRY);
  ...
  for (i = 0; i < n; ++i) {
    ...
    if (rc) {
      SDE_DEBUG("crtc%d: ctl%d reset failure\n", crtc->base.id,
                ctl->idx - CTL_0);
      SDE_EVT32(DRMID(crtc), ctl->idx - CTL_0, SDE_EVTLOG_ERROR);
      break;
    }
  }
...
static bool _sde_crtc_prepare_for_kickoff_rot(struct drm_device *dev,
  ...
  list_for_each_entry(encoder, &dev->mode_config.encoder_list, head) {
     ...
     */
    if (sde_encoder_get_intf_mode(encoder) != INTF_MODE_CMD) {
      cstate->sbuf_cfg.rot_op_mode = SDE_CTL_ROT_OP_MODE_INLINE_SYNC;
      return false;
    }
  }
...
void sde_crtc_commit_kickoff(struct drm_crtc *crtc,
   ...
   */
  if (reset_req) {
    if (_sde_crtc_reset_hw(crtc, old_state, !sde_crtc->reset_request))
      is_error = true;
  }
  ...
  if (atomic_inc_return(&sde_crtc->frame_pending) == 1) {
    ...
    SDE_EVT32(DRMID(crtc), cstate->sbuf_cfg.rot_op_mode, SDE_EVTLOG_FUNC_CASE1);
  } else {
    SDE_DEBUG("crtc%d commit\n", crtc->base.id);
    SDE_EVT32(DRMID(crtc), cstate->sbuf_cfg.rot_op_mode, SDE_EVTLOG_FUNC_CASE2);
  }
  sde_crtc->play_count++;
 ...
 * @Return: error code
 */
static int _sde_crtc_vblank_enable_no_lock(struct sde_crtc *sde_crtc,
                                           bool enable) {
  struct drm_device *dev;
  struct drm_crtc *crtc;
  struct drm_encoder *enc;
  ...
  if (enable) {
    ...
    list_for_each_entry(enc, &dev->mode_config.encoder_list, head) {
      ...
      SDE_EVT32(DRMID(&sde_crtc->base), DRMID(enc), enable, sde_crtc->enabled,
                sde_crtc->suspend, sde_crtc->vblank_requested);

      sde_encoder_register_vblank_callback(enc, sde_crtc_vblank_cb,
                                           (void *)crtc);
    }
  } else {
    list_for_each_entry(enc, &dev->mode_config.encoder_list, head) {
...
static struct drm_crtc_state *sde_crtc_duplicate_state(struct drm_crtc *crtc) {
  ...
  msm_property_duplicate_state(&sde_crtc->property_info, old_cstate, cstate,
                               &cstate->property_state,
...
static void sde_crtc_reset(struct drm_crtc *crtc) {
  ...
  _sde_crtc_rp_reset(&cstate->rp, &sde_crtc->rp_lock, &sde_crtc->rp_head);
...
static void sde_crtc_handle_power_event(u32 event_type, void *arg) {
  ...
  case SDE_POWER_EVENT_POST_ENABLE:
    ...
    for (i = 0; i < sde_crtc->num_mixers; ++i) {
      m = &sde_crtc->mixers[i];
      if (!m->hw_lm || !m->hw_lm->ops.setup_misr || !sde_crtc->misr_enable)
        continue;

      m->hw_lm->ops.setup_misr(m->hw_lm, true, sde_crtc->misr_frame_count);
    }
    break;
  case SDE_POWER_EVENT_PRE_DISABLE:
    ...
    for (i = 0; i < sde_crtc->num_mixers; ++i) {
      m = &sde_crtc->mixers[i];
      if (!m->hw_lm || !m->hw_lm->ops.collect_misr || !sde_crtc->misr_enable)
        continue;

      misr_status = m->hw_lm->ops.collect_misr(m->hw_lm);
      sde_crtc->misr_data[i] =
          misr_status ? misr_status : sde_crtc->misr_data[i];
    }
...
static void sde_crtc_disable(struct drm_crtc *crtc) {
  ...
  power_on = 0;
  msm_mode_object_event_notify(&crtc->base, crtc->dev, &event, (u8 *)&power_on);
  ...
  SDE_EVT32(DRMID(crtc), sde_crtc->enabled, sde_crtc->suspend,
            sde_crtc->vblank_requested, crtc->state->active,
            crtc->state->enable);
  if (sde_crtc->enabled && !sde_crtc->suspend && sde_crtc->vblank_requested) {
  ...
  if (sde_crtc->power_event)
    sde_power_handle_unregister_event(&priv->phandle, sde_crtc->power_event);
  ...
  if (!crtc->state->active || !crtc->state->enable) {
    sde_fence_signal(&sde_crtc->output_fence, ktime_get(),
...
static void sde_crtc_enable(struct drm_crtc *crtc) {
  ...
  drm_for_each_encoder(encoder, crtc->dev) {
    if (encoder->crtc != crtc)
      continue;
    sde_encoder_register_frame_event_callback(encoder, sde_crtc_frame_event_cb,
                                              crtc);
  }
...
static int _sde_crtc_excl_dim_layer_check(struct drm_crtc_state *state,
  ...
  for (i = 0; i < cstate->num_dim_layers; i++) {
    if ((CHECK_LAYER_BOUNDS(cstate->dim_layer[i].rect.y,
        ...
        (!cstate->dim_layer[i].rect.w) || (!cstate->dim_layer[i].rect.h)) {
      SDE_ERROR("invalid dim_layer:{%d,%d,%d,%d}, stage:%d\n",
                cstate->dim_layer[i].rect.x, cstate->dim_layer[i].rect.y,
                cstate->dim_layer[i].rect.w, cstate->dim_layer[i].rect.h,
  ...
  for (i = 0; i < cnt; i++) {
    ...
    SDE_DEBUG("p %d z %d src{%d,%d,%d,%d} excl_rect{%d,%d,%d,%d}\n",
              pstate->plane->base.id, pstates[i].stage, pstate->crtc_x,
              pstate->crtc_y, pstate->crtc_w, pstate->crtc_h,
...
static int _sde_crtc_check_secure_state(struct drm_crtc *crtc,
  ...
  if (secure == SDE_DRM_SEC_ONLY) {
     ...
     */
    if (fb_ns || fb_sec) {
      SDE_ERROR("crtc%d: invalid fb_modes Sec:%d, NS:%d, Sec_Dir:%d\n",
     ...
     */
    for (i = 1; i < cnt; i++) {
      if (!pstates[i].drm_pstate || !pstates[i].drm_pstate->plane) {
      ...
      if (pstates[i].stage != pstates[i - 1].stage) {
        SDE_ERROR("crtc%d: invalid blend stages %d:%d, %d:%d\n", crtc->base.id,
                  i, pstates[i].stage, i - 1, pstates[i - 1].stage);
        return -EINVAL;
      }
    }
  }
   ...
   */
  if (fb_sec_dir) {
    drm_for_each_encoder(encoder, crtc->dev) if (encoder->crtc == crtc)
        encoder_cnt++;
  ...
  drm_for_each_encoder(encoder, crtc->dev) {
    ...
    is_video_mode |= sde_encoder_check_mode(encoder, MSM_DISPLAY_CAP_VID_MODE);
  }
   ...
   */
  if (is_video_mode && smmu_state && state->plane_mask &&
      crtc->state->plane_mask &&
      ((fb_sec_dir &&
        ((smmu_state->state == ATTACHED) && (secure == SDE_DRM_SEC_ONLY))) ||
       (fb_ns && ((smmu_state->state == DETACHED) ||
                  (smmu_state->state == DETACH_ALL_REQ))))) {

    SDE_EVT32(DRMID(&sde_crtc->base), fb_ns, fb_sec_dir, smmu_state->state,
              crtc->state->plane_mask, crtc->state->plane_mask);
...
static int sde_crtc_get_dim_layer_alpha(struct sde_crtc_state *cstate,
  ...
  for (i = 0; i < cstate->num_connectors; i++) {
    rc = sde_connector_get_dim_layer_alpha(cstate->connectors[i], type, alpha);
    if (rc >= 0)
      break;
  }
...
static int sde_crtc_atomic_check(struct drm_crtc *crtc,
  ...
  pstates = kzalloc(SDE_PSTATES_MAX * sizeof(struct plane_state), GFP_KERNEL);
  ...
  cstate->sbuf_clk_rate[1] = sde_crtc_get_property(cstate, CRTC_PROP_ROT_CLK);
  ...
  drm_atomic_crtc_state_for_each_plane_state(plane, pstate, state) {
    ...
    pstates[cnt].stage =
        sde_plane_get_property(pstates[cnt].sde_pstate, PLANE_PROP_ZPOS);
    pstates[cnt].pipe_id = sde_plane_pipe(plane);

    /* check dim layer stage with every plane */
    for (i = 0; i < cstate->num_dim_layers; i++) {
      if (cstate->dim_layer[i].stage == (pstates[cnt].stage + SDE_STAGE_0)) {
        SDE_ERROR("plane:%d/dim_layer:%i-same stage:%d\n", plane->base.id, i,
                  cstate->dim_layer[i].stage);
        rc = -EINVAL;
        goto end;
      }
    ...
    if (CHECK_LAYER_BOUNDS(pstate->crtc_y, pstate->crtc_h, mode->vdisplay) ||
      ...
      SDE_ERROR("y:%d h:%d vdisp:%d x:%d w:%d hdisp:%d\n", pstate->crtc_y,
                pstate->crtc_h, mode->vdisplay, pstate->crtc_x, pstate->crtc_w,
                mode->hdisplay);
      rc = -E2BIG;
      goto end;
    }
  ...
  for (i = 0; i < multirect_count; i++) {
    if (sde_plane_validate_multirect_v2(&multirect_plane[i])) {
      SDE_ERROR("multirect validation failed for planes (%d - %d)\n",
  ...
  for (i = 1; i < cnt; i++) {
    ...
    left_layout =
        sde_plane_get_property(prv_pstate->sde_pstate, PLANE_PROP_LAYOUT);
    right_layout =
        sde_plane_get_property(cur_pstate->sde_pstate, PLANE_PROP_LAYOUT);
    ...
    if (right_pid < left_pid) {
      SDE_ERROR("invalid src split cfg. priority mismatch. stage: %d left: %d "
                "right: %d\n",
                stage, left_pid, right_pid);
      rc = -EINVAL;
      goto end;
    } else if (right_rect.x != (left_rect.x + left_rect.w)) {
      SDE_ERROR("non-contiguous coordinates for src split. stage: %d left: %d "
                "- %d right: %d - %d\n",
                stage, left_rect.x, left_rect.w, right_rect.x, right_rect.w);
      rc = -EINVAL;
      goto end;
    } else if ((left_rect.y != right_rect.y) || (left_rect.h != right_rect.h)) {
      SDE_ERROR("source split at stage: %d. invalid yoff/height: l_y: %d r_y: "
                "%d l_h: %d r_h: %d\n",
                stage, left_rect.y, right_rect.y, left_rect.h, right_rect.h);
      rc = -EINVAL;
      goto end;
    }
  }
...
int sde_crtc_vblank(struct drm_crtc *crtc, bool en) {
  ...
  mutex_lock(&sde_crtc->crtc_lock);
  SDE_EVT32(DRMID(&sde_crtc->base), en, sde_crtc->enabled, sde_crtc->suspend,
            sde_crtc->vblank_requested);
...
int sde_crtc_helper_reset_custom_properties(struct drm_crtc *crtc,
  ...
  for (prop_idx = 0; prop_idx < CRTC_PROP_COUNT; prop_idx++) {
    ...
    drm_prop =
        msm_property_index_to_drm_property(&sde_crtc->property_info, prop_idx);
    if (!drm_prop) {
    ...
    def = msm_property_get_default(&sde_crtc->property_info, prop_idx);
    if (val == def)
      continue;
    ...
    ret = drm_atomic_crtc_set_property(crtc, crtc_state, drm_prop, def);
    if (ret) {
...
static void sde_crtc_install_properties(struct drm_crtc *crtc,
  ...
  /* range properties */
  msm_property_install_range(&sde_crtc->property_info, "input_fence_timeout",
                             0x0, 0, SDE_CRTC_MAX_INPUT_FENCE_TIMEOUT,
                             SDE_CRTC_INPUT_FENCE_TIMEOUT,
                             CRTC_PROP_INPUT_FENCE_TIMEOUT);

  msm_property_install_volatile_range(&sde_crtc->property_info, "output_fence",
                                      0x0, 0, ~0, 0, CRTC_PROP_OUTPUT_FENCE);

  msm_property_install_range(&sde_crtc->property_info, "output_fence_offset",
                             0x0, 0, 1, 0, CRTC_PROP_OUTPUT_FENCE_OFFSET);

  msm_property_install_range(&sde_crtc->property_info, "core_clk", 0x0, 0,
                             U64_MAX, sde_kms->perf.max_core_clk_rate,
                             CRTC_PROP_CORE_CLK);
  msm_property_install_range(&sde_crtc->property_info, "core_ab", 0x0, 0,
                             U64_MAX, catalog->perf.max_bw_high * 1000ULL,
                             CRTC_PROP_CORE_AB);
  msm_property_install_range(&sde_crtc->property_info, "core_ib", 0x0, 0,
                             U64_MAX, catalog->perf.max_bw_high * 1000ULL,
                             CRTC_PROP_CORE_IB);
  msm_property_install_range(&sde_crtc->property_info, "llcc_ab", 0x0, 0,
                             U64_MAX, catalog->perf.max_bw_high * 1000ULL,
                             CRTC_PROP_LLCC_AB);
  msm_property_install_range(&sde_crtc->property_info, "llcc_ib", 0x0, 0,
                             U64_MAX, catalog->perf.max_bw_high * 1000ULL,
                             CRTC_PROP_LLCC_IB);
  msm_property_install_range(&sde_crtc->property_info, "dram_ab", 0x0, 0,
                             U64_MAX, catalog->perf.max_bw_high * 1000ULL,
                             CRTC_PROP_DRAM_AB);
  msm_property_install_range(&sde_crtc->property_info, "dram_ib", 0x0, 0,
                             U64_MAX, catalog->perf.max_bw_high * 1000ULL,
                             CRTC_PROP_DRAM_IB);
  msm_property_install_range(&sde_crtc->property_info, "rot_prefill_bw", 0, 0,
                             U64_MAX, catalog->perf.max_bw_high * 1000ULL,
                             CRTC_PROP_ROT_PREFILL_BW);
  msm_property_install_range(&sde_crtc->property_info, "rot_clk", 0, 0, U64_MAX,
                             sde_kms->perf.max_core_clk_rate,
                             CRTC_PROP_ROT_CLK);

  msm_property_install_range(&sde_crtc->property_info, "idle_time", 0, 0,
                             U64_MAX, 0, CRTC_PROP_IDLE_TIMEOUT);

  msm_property_install_range(&sde_crtc->property_info, "enable_sui_enhancement",
                             0, 0, U64_MAX, 0,
                             CRTC_PROP_ENABLE_SUI_ENHANCEMENT);

  if (catalog->has_idle_pc)
    msm_property_install_enum(&sde_crtc->property_info, "idle_pc_state", 0x0, 0,
                              e_idle_pc_state, ARRAY_SIZE(e_idle_pc_state),
                              CRTC_PROP_IDLE_PC_STATE);

  if (catalog->has_cwb_support)
    msm_property_install_enum(&sde_crtc->property_info, "capture_mode", 0, 0,
                              e_cwb_data_points, ARRAY_SIZE(e_cwb_data_points),
  ...
  msm_property_install_volatile_range(&sde_crtc->property_info,
                                      "sde_drm_roi_v1", 0x0, 0, ~0, 0,
                                      CRTC_PROP_ROI_V1);

  msm_property_install_enum(&sde_crtc->property_info, "security_level", 0x0, 0,
                            e_secure_level, ARRAY_SIZE(e_secure_level),
  ...
  sde_kms_info_add_keyint(info, "max_linewidth", catalog->max_mixer_width);
  ...
  if (sde_is_custom_client()) {
    if (catalog->smart_dma_rev == SDE_SSPP_SMART_DMA_V1)
      sde_kms_info_add_keystr(info, "smart_dma_rev", "smart_dma_v1");
    if (catalog->smart_dma_rev == SDE_SSPP_SMART_DMA_V2)
      sde_kms_info_add_keystr(info, "smart_dma_rev", "smart_dma_v2");
  }

  if (catalog->mdp[0].has_dest_scaler) {
    sde_kms_info_add_keyint(info, "has_dest_scaler",
                            catalog->mdp[0].has_dest_scaler);
    sde_kms_info_add_keyint(info, "dest_scaler_count", catalog->ds_count);

    if (catalog->ds[0].top) {
      sde_kms_info_add_keyint(info, "max_dest_scaler_input_width",
                              catalog->ds[0].top->maxinputwidth);
      sde_kms_info_add_keyint(info, "max_dest_scaler_output_width",
                              catalog->ds[0].top->maxinputwidth);
    ...
    if (catalog->ds[0].features & BIT(SDE_SSPP_SCALER_QSEED3)) {
      msm_property_install_volatile_range(&sde_crtc->property_info,
                                          "dest_scaler", 0x0, 0, ~0, 0,
                                          CRTC_PROP_DEST_SCALER);
      msm_property_install_blob(&sde_crtc->property_info, "ds_lut_ed", 0,
                                CRTC_PROP_DEST_SCALER_LUT_ED);
      msm_property_install_blob(&sde_crtc->property_info, "ds_lut_cir", 0,
                                CRTC_PROP_DEST_SCALER_LUT_CIR);
      msm_property_install_blob(&sde_crtc->property_info, "ds_lut_sep", 0,
  ...
  if (sde_kms->perf.max_core_clk_rate)
    sde_kms_info_add_keyint(info, "max_mdp_clk",
                            sde_kms->perf.max_core_clk_rate);
  sde_kms_info_add_keystr(info, "core_ib_ff", catalog->perf.core_ib_ff);
  sde_kms_info_add_keystr(info, "core_clk_ff", catalog->perf.core_clk_ff);
  sde_kms_info_add_keystr(info, "comp_ratio_rt", catalog->perf.comp_ratio_rt);
  sde_kms_info_add_keystr(info, "comp_ratio_nrt", catalog->perf.comp_ratio_nrt);
...
static int _sde_crtc_get_output_fence(struct drm_crtc *crtc,
  ...
  drm_for_each_encoder(encoder, crtc->dev) {
    if (encoder->crtc == crtc)
      is_cmd = sde_encoder_check_mode(encoder, MSM_DISPLAY_CAP_CMD_MODE);
  }
...
static int sde_crtc_atomic_set_property(struct drm_crtc *crtc,
  ...
  case CRTC_PROP_DIM_LAYER_V1:
    _sde_crtc_set_dim_layer_v1(cstate, (void __user *)(uintptr_t)val);
    break;
  case CRTC_PROP_ROI_V1:
    ret = _sde_crtc_set_roi_v1(state, (void __user *)(uintptr_t)val);
    break;
...
static int _sde_debugfs_status_show(struct seq_file *s, void *data) {
  ...
  for (i = 0; i < sde_crtc->num_mixers; ++i) {
    ...
    if (!m->hw_lm)
      ...
      seq_printf(s, "\tmixer:%d ctl:%d width:%d height:%d\n",
                 m->hw_lm->idx - LM_0, m->hw_ctl->idx - CTL_0, out_width,
                 out_height);
  }
  ...
  for (i = 0; i < cstate->num_dim_layers; i++) {
    ...
    seq_printf(s, "\tdst_x:%d dst_y:%d dst_w:%d dst_h:%d\n", dim_layer->rect.x,
               dim_layer->rect.y, dim_layer->rect.w, dim_layer->rect.h);
    seq_printf(s, "\tcolor_0:%d color_1:%d color_2:%d color_3:%d\n",
               dim_layer->color_fill.color_0, dim_layer->color_fill.color_1,
               dim_layer->color_fill.color_2, dim_layer->color_fill.color_3);
    seq_puts(s, "\n");
  }
  ...
  drm_atomic_crtc_for_each_plane(plane, crtc) {
    ...
    seq_printf(s, "\tplane:%u stage:%d\n", plane->base.id, pstate->stage);

    if (plane->state->fb) {
      fb = plane->state->fb;

      seq_printf(s, "\tfb:%d image format:%4.4s wxh:%ux%u bpp:%d\n",
                 fb->base.id, (char *)&fb->pixel_format, fb->width, fb->height,
                 fb->bits_per_pixel);

      seq_puts(s, "\t");
      for (i = 0; i < ARRAY_SIZE(fb->modifier); i++)
        seq_printf(s, "modifier[%d]:%8llu ", i, fb->modifier[i]);
      seq_puts(s, "\n");

      seq_puts(s, "\t");
      for (i = 0; i < ARRAY_SIZE(fb->pitches); i++)
        seq_printf(s, "pitches[%d]:%8u ", i, fb->pitches[i]);
      seq_puts(s, "\n");

      seq_puts(s, "\t");
      for (i = 0; i < ARRAY_SIZE(fb->offsets); i++)
        seq_printf(s, "offsets[%d]:%8u ", i, fb->offsets[i]);
      seq_puts(s, "\n");
    }

    seq_printf(s, "\tsrc_x:%4d src_y:%4d src_w:%4d src_h:%4d\n", state->src_x,
               state->src_y, state->src_w, state->src_h);
    ...
    seq_printf(s, "\texcl_rect: x:%4d y:%4d w:%4d h:%4d\n", pstate->excl_rect.x,
               pstate->excl_rect.y, pstate->excl_rect.w, pstate->excl_rect.h);
  ...
  if (sde_crtc->vblank_cb_count) {
    ...
    u64 fps = diff_ms
                  ? DIV_ROUND_CLOSEST(sde_crtc->vblank_cb_count * 1000, diff_ms)
                  : 0;

    seq_printf(
        s, "vblank fps:%lld count:%u total:%llums total_framecount:%llu\n", fps,
        sde_crtc->vblank_cb_count, ktime_to_ms(diff), sde_crtc->play_count);
...
static ssize_t _sde_crtc_misr_read(struct file *file, char __user *user_buff,
  ...
  for (i = 0; i < sde_crtc->num_mixers; ++i) {
    ...
    misr_status = m->hw_lm->ops.collect_misr(m->hw_lm);
    sde_crtc->misr_data[i] = misr_status ? misr_status : sde_crtc->misr_data[i];
...
static int sde_crtc_debugfs_state_show(struct seq_file *s, void *v) {
  ...
  for (i = SDE_POWER_HANDLE_DBUS_ID_MNOC; i < SDE_POWER_HANDLE_DBUS_ID_MAX;
       i++) {
  ...
  list_for_each_entry(rp, &sde_crtc->rp_head, rp_list) {
    ...
    list_for_each_entry(res, &rp->res_list, list)
        seq_printf(s, "0x%x/0x%llx/%pK/%d ", res->type, res->tag, res->val,
                   atomic_read(&res->refcount));
    seq_puts(s, "\n");
  }
...
static int _sde_debugfs_fence_status(struct inode *inode, struct file *file) {
  return single_open(file, _sde_debugfs_fence_status_show, inode->i_private);
}
...
static int _sde_crtc_init_debugfs(struct drm_crtc *crtc) {
  ...
  sde_crtc->debugfs_root =
      debugfs_create_dir(sde_crtc->name, crtc->dev->primary->debugfs_root);
  if (!sde_crtc->debugfs_root)
    return -ENOMEM;

  /* don't error check these */
  debugfs_create_file("status", 0400, sde_crtc->debugfs_root, sde_crtc,
                      &debugfs_status_fops);
  debugfs_create_file("state", 0600, sde_crtc->debugfs_root, &sde_crtc->base,
                      &sde_crtc_debugfs_state_fops);
  debugfs_create_file("misr_data", 0600, sde_crtc->debugfs_root, sde_crtc,
                      &debugfs_misr_fops);
  debugfs_create_file("fence_status", 0400, sde_crtc->debugfs_root, sde_crtc,
                      &debugfs_fence_fops);
  debugfs_create_file("fps", 0400, sde_crtc->debugfs_root, sde_crtc,
                      &debugfs_fps_fops);
...
#else
static int _sde_crtc_init_debugfs(struct drm_crtc *crtc) { return 0; }
...
int sde_crtc_event_queue(struct drm_crtc *crtc,
  ...
  if (!list_empty(&sde_crtc->event_free_list)) {
    event = list_first_entry(&sde_crtc->event_free_list, struct sde_crtc_event,
                             list);
  ...
  if (color_processing_event)
    kthread_queue_work(&priv->pp_event_worker, &event->kt_work);
  else
    kthread_queue_work(&priv->event_thread[crtc_id].worker, &event->kt_work);
...
static int _sde_crtc_init_events(struct sde_crtc *sde_crtc) {
  ...
  for (i = 0; i < SDE_CRTC_MAX_EVENT_COUNT; ++i)
    list_add_tail(&sde_crtc->event_cache[i].list, &sde_crtc->event_free_list);
...
static void __sde_crtc_idle_notify_work(struct kthread_work *work) {
  struct sde_crtc *sde_crtc =
      container_of(work, struct sde_crtc, idle_notify_work.work);
  struct drm_crtc *crtc;
  struct drm_event event;
  int ret = 0;
  ...
  if (!sde_crtc) {
    ...
    event.length = sizeof(u32);
    msm_mode_object_event_notify(&crtc->base, crtc->dev, &event, (u8 *)&ret);
...
struct drm_crtc *sde_crtc_init(struct drm_device *dev,
  ...
  for (i = 0; i < ARRAY_SIZE(sde_crtc->frame_events); i++) {
    ...
    list_add(&sde_crtc->frame_events[i].list, &sde_crtc->frame_event_list);
...
int sde_crtc_post_init(struct drm_device *dev, struct drm_crtc *crtc) {
  ...
  sde_crtc->vsync_event_sf =
      sysfs_get_dirent(sde_crtc->sysfs_dev->kobj.sd, "vsync_event");
  if (!sde_crtc->vsync_event_sf)
...
static int _sde_crtc_event_enable(struct sde_kms *kms,
  ...
  for (i = 0; i < ARRAY_SIZE(custom_events); i++) {
    if (custom_events[i].event == event && custom_events[i].func) {
 ...
 * @crtc: Pointer to drm crtc structure
 */
void sde_crtc_update_cont_splash_mixer_settings(struct drm_crtc *crtc) {
  _sde_crtc_setup_mixers(crtc);
  crtc->enabled = true;
}
