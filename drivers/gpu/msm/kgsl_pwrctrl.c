 ...
 */
static void _record_pwrevent(struct kgsl_device *device, ktime_t t, int event) {
  ...
  if (history->events == NULL)
    return;
  history->events[i].duration = ktime_us_delta(t, history->events[i].start);
  i = (i + 1) % history->size;
  history->index = i;
 ...
 */
static unsigned long kgsl_get_bw(void) { return ib_votes[last_vote_buslevel]; }
#endif
 ...
 */
static void _ab_buslevel_update(struct kgsl_pwrctrl *pwr, unsigned long *ab) {
...
static unsigned int _adjust_pwrlevel(struct kgsl_pwrctrl *pwr, int level,
  ...
  unsigned int min_pwrlevel =
      min_t(unsigned int, pwr->thermal_pwrlevel_floor, pwr->min_pwrlevel);
 ...
 */
int kgsl_clk_set_rate(struct kgsl_device *device, unsigned int pwrlevel) {
  ...
  if (kgsl_gmu_isenabled(device)) {
    ...
    ret = kgsl_pwrctrl_clk_set_rate(pwr->grp_clks[0], pl->gpu_freq, clocks[0]);
...
void kgsl_pwrctrl_buslevel_update(struct kgsl_device *device, bool on) {
   ...
   */
  if (on) {
    buslevel = min_t(int, pwr->pwrlevels[0].bus_max, cur + pwr->bus_mod);
  ...
  if (pwr->ocmem_pcl)
    msm_bus_scale_client_update_request(
        pwr->ocmem_pcl, on ? pwr->active_pwrlevel : pwr->num_pwrlevels - 1);
...
void kgsl_pwrctrl_set_thermal_cycle(struct kgsl_device *device,
  ...
  if (pwr->thermal_pwrlevel == pwr->sysfs_pwr_limit->level) {
    ...
    if (pwr->thermal_cycle == CYCLE_ENABLE) {
      ...
      mod_timer(&pwr->thermal_timer, jiffies + (TH_HZ - pwr->thermal_timeout));
...
unsigned int kgsl_pwrctrl_adjust_pwrlevel(struct kgsl_device *device,
  ...
  if ((pwr->constraint.type != KGSL_CONSTRAINT_NONE) &&
    ...
    trace_kgsl_constraint(device, pwr->constraint.type, old_level, 0);
...
void kgsl_pwrctrl_pwrlevel_change(struct kgsl_device *device,
  ...
  if (new_level == old_level && !test_bit(GMU_DCVS_REPLAY, &device->gmu.flags))
    return;
  ...
  if (pwr->gpu_bimc_int_clk) {
    if (pwr->active_pwrlevel == 0 && !pwr->gpu_bimc_interface_enabled) {
      kgsl_pwrctrl_clk_set_rate(pwr->gpu_bimc_int_clk,
                                pwr->gpu_bimc_int_clk_freq, "bimc_gpu_clk");
      _bimc_clk_prepare_enable(device, pwr->gpu_bimc_int_clk, "bimc_gpu_clk");
...
void kgsl_pwrctrl_update_l2pc(struct kgsl_device *device,
  ...
  if ((1 << cpu) & device->pwrctrl.l2pc_cpus_mask) {
    pm_qos_update_request_timeout(&device->pwrctrl.l2pc_cpus_qos,
...
static ssize_t
kgsl_pwrctrl_thermal_pwrlevel_store(struct device *dev,
...
static unsigned int kgsl_pwrctrl_max_clock_get(struct kgsl_device *device) {
  ...
  if (pwr->thermal_cycle) {
    ...
    unsigned int lfreq = pwr->pwrlevels[pwr->thermal_pwrlevel + 1].gpu_freq;
...
static ssize_t
kgsl_pwrctrl_pmqos_active_latency_store(struct device *dev,
                                        struct device_attribute *attr,
                                        const char *buf, size_t count) {
  unsigned int val = 0;
  struct kgsl_device *device = kgsl_device_from_dev(dev);
...
static ssize_t kgsl_pwrctrl_pmqos_active_latency_show(
...
static ssize_t kgsl_pwrctrl_gpu_available_frequencies_show(
  ...
  for (index = 0; index < pwr->num_pwrlevels - 1; index++) {
    num_chars += scnprintf(buf + num_chars, PAGE_SIZE - num_chars - 1, "%d ",
...
static ssize_t kgsl_pwrctrl_gpu_clock_stats_show(struct device *dev,
                                                 struct device_attribute *attr,
                                                 char *buf) {
  struct kgsl_device *device = kgsl_device_from_dev(dev);
  struct kgsl_pwrctrl *pwr;
  ...
  mutex_unlock(&device->mutex);
  for (index = 0; index < pwr->num_pwrlevels - 1; index++)
    num_chars += snprintf(buf + num_chars, PAGE_SIZE - num_chars, "%llu ",
                          pwr->clock_times[index]);
...
static ssize_t
kgsl_pwrctrl_default_pwrlevel_store(struct device *dev,
                                    struct device_attribute *attr,
                                    const char *buf, size_t count) {
  struct kgsl_device *device = kgsl_device_from_dev(dev);
  struct kgsl_pwrctrl *pwr;
  ...
  pwrscale->gpu_profile.profile.initial_freq = pwr->pwrlevels[level].gpu_freq;
...
static ssize_t kgsl_popp_show(struct device *dev, struct device_attribute *attr,
                              char *buf) {
  struct kgsl_device *device = kgsl_device_from_dev(dev);
...
static ssize_t kgsl_pwrctrl_freq_table_mhz_show(struct device *dev,
                                                struct device_attribute *attr,
                                                char *buf) {
  struct kgsl_device *device = kgsl_device_from_dev(dev);
  struct kgsl_pwrctrl *pwr;
...
static ssize_t kgsl_pwrctrl_temp_show(struct device *dev,
  ...
  for (i = 0; i < KGSL_MAX_TZONE_NAMES; i++) {
    ...
    thermal_dev = thermal_zone_get_zone_by_name(pwr->tzone_names[i]);
...
static DEVICE_ATTR(gpubusy, 0444, kgsl_pwrctrl_gpubusy_show, NULL);
static DEVICE_ATTR(gpu_available_frequencies, 0444,
                   kgsl_pwrctrl_gpu_available_frequencies_show, NULL);
static DEVICE_ATTR(gpu_clock_stats, 0444, kgsl_pwrctrl_gpu_clock_stats_show,
                   NULL);
static DEVICE_ATTR(max_pwrlevel, 0644, kgsl_pwrctrl_max_pwrlevel_show,
                   kgsl_pwrctrl_max_pwrlevel_store);
static DEVICE_ATTR(min_pwrlevel, 0644, kgsl_pwrctrl_min_pwrlevel_show,
                   kgsl_pwrctrl_min_pwrlevel_store);
static DEVICE_ATTR(thermal_pwrlevel, 0644, kgsl_pwrctrl_thermal_pwrlevel_show,
                   kgsl_pwrctrl_thermal_pwrlevel_store);
static DEVICE_ATTR(num_pwrlevels, 0444, kgsl_pwrctrl_num_pwrlevels_show, NULL);
...
static DEVICE_ATTR(reset_count, 0444, kgsl_pwrctrl_reset_count_show, NULL);
static DEVICE_ATTR(force_clk_on, 0644, kgsl_pwrctrl_force_clk_on_show,
                   kgsl_pwrctrl_force_clk_on_store);
static DEVICE_ATTR(force_bus_on, 0644, kgsl_pwrctrl_force_bus_on_show,
                   kgsl_pwrctrl_force_bus_on_store);
static DEVICE_ATTR(force_rail_on, 0644, kgsl_pwrctrl_force_rail_on_show,
                   kgsl_pwrctrl_force_rail_on_store);
static DEVICE_ATTR(bus_split, 0644, kgsl_pwrctrl_bus_split_show,
                   kgsl_pwrctrl_bus_split_store);
static DEVICE_ATTR(default_pwrlevel, 0644, kgsl_pwrctrl_default_pwrlevel_show,
                   kgsl_pwrctrl_default_pwrlevel_store);
static DEVICE_ATTR(popp, 0644, kgsl_popp_show, kgsl_popp_store);
static DEVICE_ATTR(force_no_nap, 0644, kgsl_pwrctrl_force_no_nap_show,
...
static DEVICE_ATTR(freq_table_mhz, 0444, kgsl_pwrctrl_freq_table_mhz_show,
                   NULL);
static DEVICE_ATTR(pwrscale, 0644, kgsl_pwrctrl_pwrscale_show,
...
int kgsl_pwrctrl_init_sysfs(struct kgsl_device *device) {
  ...
  if (IS_ERR_OR_NULL(device->gpu_sysfs_kobj))
    return (device->gpu_sysfs_kobj == NULL) ? -ENOMEM
                                            : PTR_ERR(device->gpu_sysfs_kobj);

  for (i = 0; i < ARRAY_SIZE(link_names); i++)
    kgsl_gpu_sysfs_add_link(device->gpu_sysfs_kobj, &device->dev->kobj,
                            link_names[i].src, link_names[i].dst);
...
static void kgsl_pwrctrl_clk(struct kgsl_device *device, int state,
  ...
  if (state == KGSL_PWRFLAGS_OFF) {
    if (test_and_clear_bit(KGSL_PWRFLAGS_CLK_ON, &pwr->power_flags)) {
      trace_kgsl_clk(device, state, kgsl_pwrctrl_active_freq(pwr));
      /* Disable gpu-bimc-interface clocks */
      if (pwr->gpu_bimc_int_clk && pwr->gpu_bimc_interface_enabled) {
        clk_disable_unprepare(pwr->gpu_bimc_int_clk);
        pwr->gpu_bimc_interface_enabled = 0;
      }
    ...
    if (!test_and_set_bit(KGSL_PWRFLAGS_CLK_ON, &pwr->power_flags)) {
      trace_kgsl_clk(device, state, kgsl_pwrctrl_active_freq(pwr));
      /* High latency clock maintenance. */
      ...
      for (i = KGSL_MAX_CLKS - 1; i > 0; i--)
        _gpu_clk_prepare_enable(device, pwr->grp_clks[i], clocks[i]);
...
static void kgsl_pwrctrl_axi(struct kgsl_device *device, int state) {
  ...
  if (state == KGSL_PWRFLAGS_OFF) {
    if (test_and_clear_bit(KGSL_PWRFLAGS_AXI_ON, &pwr->power_flags)) {
    ...
    if (!test_and_set_bit(KGSL_PWRFLAGS_AXI_ON, &pwr->power_flags)) {
      trace_kgsl_bus(device, state);
      kgsl_pwrctrl_buslevel_update(device, true);
...
static int kgsl_pwrctrl_pwrrail(struct kgsl_device *device, int state) {
  ...
  if (state == KGSL_PWRFLAGS_OFF) {
    if (test_and_clear_bit(KGSL_PWRFLAGS_POWER_ON, &pwr->power_flags)) {
...
static void kgsl_pwrctrl_irq(struct kgsl_device *device, int state) {
  ...
  if (state == KGSL_PWRFLAGS_ON) {
    if (!test_and_set_bit(KGSL_PWRFLAGS_IRQ_ON, &pwr->power_flags)) {
      trace_kgsl_irq(device, state);
      enable_irq(pwr->interrupt_num);
    }
  } else if (state == KGSL_PWRFLAGS_OFF) {
    if (test_and_clear_bit(KGSL_PWRFLAGS_IRQ_ON, &pwr->power_flags)) {
      trace_kgsl_irq(device, state);
      if (in_interrupt())
...
static void kgsl_thermal_cycle(struct work_struct *work) {
  struct kgsl_pwrctrl *pwr =
      container_of(work, struct kgsl_pwrctrl, thermal_cycle_ws);
  ...
  if (pwr->thermal_cycle == CYCLE_ACTIVE) {
    if (pwr->thermal_highlow)
      ...
      kgsl_pwrctrl_pwrlevel_change(device, pwr->thermal_pwrlevel + 1);
  }
  mutex_unlock(&device->mutex);
}
...
static void kgsl_thermal_timer(unsigned long data) {
  ...
  if (device->pwrctrl.thermal_highlow) {
    mod_timer(&device->pwrctrl.thermal_timer,
              jiffies + device->pwrctrl.thermal_timeout);
    device->pwrctrl.thermal_highlow = 0;
  } else {
    mod_timer(&device->pwrctrl.thermal_timer,
              jiffies + (TH_HZ - device->pwrctrl.thermal_timeout));
...
#else
static int kgsl_pwrctrl_vbif_init(void) { return 0; }
#endif
...
static int get_regulators(struct kgsl_device *device) {
  ...
  of_property_for_each_string(dev->of_node, "regulator-names", prop, name) {
    int ret;
...
static int _isense_clk_set_rate(struct kgsl_pwrctrl *pwr, int level) {
  ...
  rate =
      clk_round_rate(pwr->grp_clks[pwr->isense_clk_indx],
                     level > pwr->isense_clk_on_level ? KGSL_XO_CLK_FREQ
                                                      : KGSL_ISENSE_CLK_FREQ);
  return kgsl_pwrctrl_clk_set_rate(pwr->grp_clks[pwr->isense_clk_indx], rate,
...
int kgsl_pwrctrl_init(struct kgsl_device *device) {
  ...
  if (!of_property_read_u32(pdev->dev.of_node,
    ...
    pwr->gpu_bimc_int_clk = devm_clk_get(&pdev->dev, "bimc_gpu_clk");
  ...
  kgsl_property_read_u32(device, "qcom,l2pc-cpu-mask", &pwr->l2pc_cpus_mask);

  pwr->l2pc_update_queue = of_property_read_bool(device->pdev->dev.of_node,
                                                 "qcom,l2pc-update-queue");

  pm_runtime_enable(&pdev->dev);

  ocmem_bus_node =
      of_find_node_by_name(device->pdev->dev.of_node, "qcom,ocmem-bus-client");
  /* If platform has split ocmem bus client - use it */
  if (ocmem_bus_node) {
    ocmem_scale_table = msm_bus_pdata_from_node(device->pdev, ocmem_bus_node);
    if (ocmem_scale_table)
      pwr->ocmem_pcl = msm_bus_scale_register_client(ocmem_scale_table);
  ...
  if (pwr->bus_control)
    /* Check if gpu bandwidth vote device is defined in dts */
    gpubw_dev_node = of_parse_phandle(pdev->dev.of_node, "qcom,gpubw-dev", 0);
  ...
  pwr->bus_ib =
      kzalloc(bus_scale_table->num_usecases * sizeof(*pwr->bus_ib), GFP_KERNEL);
  if (pwr->bus_ib == NULL) {
    result = -ENOMEM;
  ...
  for (i = 0; i < bus_scale_table->num_usecases; i++) {
    ...
    if (vector->dst == MSM_BUS_SLAVE_EBI_CH0 && vector->ib != 0) {

      if (i < KGSL_MAX_BUSLEVELS) {
        /* Convert bytes to Mbytes. */
        ib_votes[i] = DIV_ROUND_UP_ULL(vector->ib, 1048576) - 1;
      ...
      if (k == n) {
        ...
        for (m = 0; m < pwr->num_pwrlevels - 1; m++) {
          if (bus_scale_table->usecase[pwr->pwrlevels[m].bus_freq]
                  .vectors[0]
                  .ib == vector->ib)
            pwr->bus_index[m] = k;
        }
  ...
  setup_timer(&pwr->thermal_timer, kgsl_thermal_timer, (unsigned long)device);
...
void kgsl_idle_check(struct work_struct *work) {
  ...
  /*
   * BACKPORT:[Cursor] KGSL Idle Opt - Early exit when requested
   * state is NONE. Backported from gfx-kernel.lnx.1.0.r2-rel.
   * After scheduling idle work, a new workload may arrive and
   * clear the requested state to NONE before we get the mutex.
   * In that case, skip the entire idle check to avoid wasted
   * CPU cycles and spurious state transitions.
   */
  ...
  if (device->state == KGSL_STATE_ACTIVE || device->state == KGSL_STATE_NAP) {
    ...
    if (!atomic_read(&device->active_cnt)) {
      ...
      ret = kgsl_pwrctrl_change_state(device, device->requested_state);
      if (ret == -EBUSY) {
        if (requested_state == KGSL_STATE_SLUMBER) {
          spin_lock(&device->submit_lock);
    ...
    if (device->state == KGSL_STATE_ACTIVE)
      /*
       * BACKPORT:[Cursor] KGSL Idle Opt - Halve the
       * re-arm timer after an idle check that kept the
       * GPU active. This makes re-evaluation faster,
       * accelerating the path to SLUMBER.
       */
      mod_timer(&device->idle_timer,
                jiffies + (device->pwrctrl.interval_timeout >> 1));
  }
...
static void kgsl_pwrctrl_clk_set_options(struct kgsl_device *device, bool on) {
  ...
  for (i = 0; i < KGSL_MAX_CLKS; i++) {
    ...
    if (device->ftbl->clk_set_options)
      device->ftbl->clk_set_options(device, clocks[i], pwr->grp_clks[i], on);
  }
}
...
static int _aware(struct kgsl_device *device) {
  ...
  case KGSL_STATE_INIT:
    ...
    if (kgsl_gmu_isenabled(device) && test_bit(GMU_FAULT, &gmu->flags)) {
      status = -EINVAL;
      break;
    }
    status = kgsl_pwrctrl_enable(device);
...
static int _nap(struct kgsl_device *device) {
  ...
  case KGSL_STATE_RESET:
    break;
  case KGSL_STATE_AWARE:
    KGSL_PWR_WARN(device, "transition AWARE -> NAP is not permitted\n");
    /* fallthrough */
  default:
    kgsl_pwrctrl_request_state(device, KGSL_STATE_NONE);
    break;
...
static int _slumber(struct kgsl_device *device) {
  ...
  case KGSL_STATE_NAP:
    ...
    if (device->pwrctrl.l2pc_cpus_mask)
      pm_qos_update_request(&device->pwrctrl.l2pc_cpus_qos,
                            PM_QOS_DEFAULT_VALUE);
    break;
  case KGSL_STATE_SUSPEND:
    complete_all(&device->hwaccess_gate);
...
int kgsl_active_count_get(struct kgsl_device *device) {
  ...
  trace_kgsl_active_count(device, (unsigned long)__builtin_return_address(0));
  return ret;
}
...
void kgsl_active_count_put(struct kgsl_device *device) {
  ...
  if (atomic_dec_and_test(&device->active_cnt)) {
    bool nap_on = !(device->pwrctrl.ctrl_flags & BIT(KGSL_PWRFLAGS_NAP_OFF));
    /*
     * BACKPORT:[Cursor] KGSL Idle Opt - Request SLUMBER
     * directly instead of NAP when active_cnt drops to 0.
     * This bypasses the intermediate NAP state, allowing
     * the GPU to enter deep sleep faster when the render
     * queue is empty. The _slumber() path still checks
     * is_hw_collapsible() to prevent unsafe transitions.
     */
...
void *kgsl_pwr_limits_add(enum kgsl_deviceid id) {
  ...
  limit = kzalloc(sizeof(struct kgsl_pwr_limit), GFP_KERNEL);
  if (limit == NULL)
