...
void kgsl_pwrscale_sleep(struct kgsl_device *device) {
  ...
  queue_work(device->pwrscale.devfreq_wq, &device->pwrscale.devfreq_suspend_ws);
}
...
void kgsl_pwrscale_wake(struct kgsl_device *device) {
  ...
  psc->next_governor_call =
      ktime_add_us(psc->time, KGSL_GOVERNOR_CALL_INTERVAL);
...
void kgsl_pwrscale_update_stats(struct kgsl_device *device) {
  ...
  if (device->state == KGSL_STATE_ACTIVE) {
    ...
    pwrctrl->clock_times[pwrctrl->active_pwrlevel] += stats.busy_time;
  }
}
...
void kgsl_pwrscale_update(struct kgsl_device *device) {
  ...
  /*
   * BACKPORT:[Cursor] KGSL Idle Opt - Fast-path for NAP state.
   * When the GPU is already in NAP (clocks off), bypass the
   * governor call interval time-gate and immediately notify
   * devfreq. This ensures the governor sees the idle state
   * promptly and can ramp down frequency without waiting for
...
notify:
  device->pwrscale.next_governor_call =
      ktime_add_us(t, KGSL_GOVERNOR_CALL_INTERVAL);
...
void kgsl_pwrscale_midframe_timer_restart(struct kgsl_device *device) {
  if (kgsl_midframe) {
    ...
    hrtimer_start(&kgsl_midframe->timer,
                  ns_to_ktime(KGSL_GOVERNOR_CALL_INTERVAL * NSEC_PER_USEC),
                  HRTIMER_MODE_REL);
  }
}
...
kgsl_pwrscale_midframe_timer(struct hrtimer *timer) {
  struct kgsl_device *device = kgsl_midframe->device;

  queue_work(device->pwrscale.devfreq_wq, &kgsl_midframe->timer_check_ws);
...
static bool popp_stable(struct kgsl_device *device) {
  ...
  if (test_bit(POPP_PUSH, &psc->popp_state))
    return false;
  if (!psc->popp_level && (pwr->active_pwrlevel == pwr->min_pwrlevel))
    return false;
  ...
  if (pwr->active_pwrlevel == 0) {
    ...
    i = index > 0 ? (index - 1) : (psc->history[KGSL_PWREVENT_STATE].size - 1);
    while (i != index) {
      e = &psc->history[KGSL_PWREVENT_STATE].events[i];
      if (e->data == KGSL_STATE_NAP || e->data == KGSL_STATE_SLUMBER) {
    ...
    trace_kgsl_popp_nap(device, (int)nap_time / 1000, nap, percent_nap);
...
bool kgsl_popp_check(struct kgsl_device *device) {
  ...
  for (i = 0; i < psc->history[KGSL_PWREVENT_STATE].size; i++) {
    e = &psc->history[KGSL_PWREVENT_STATE].events[i];
    if ((e->data == KGSL_STATE_SLUMBER) && (e->duration > POPP_RESET_TIME)) {
      clear_bit(POPP_PUSH, &psc->popp_state);
      return false;
    }
  }
...
#else
static inline bool _check_maxfreq(u32 flags) { return false; }
#endif
...
int kgsl_devfreq_target(struct device *dev, unsigned long *freq, u32 flags) {
  ...
  if (rec_freq != cur_freq) {
     ...
     */
    for (i = pwr->min_pwrlevel;
         (i >= pwr->max_pwrlevel && i <= pwr->min_pwrlevel); i--)
...
int kgsl_devfreq_get_dev_status(struct device *dev,
  ...
  if (pwrctrl->bus_control) {
    struct xstats *last_b = (struct xstats *)last_status.private_data;
...
#else
static inline bool _check_fast_hint(u32 flags) { return false; }
#endif
...
#else
static inline bool _check_slow_hint(u32 flags) { return false; }
#endif
...
static int opp_notify(struct notifier_block *nb, unsigned long type,
                      void *in_opp) {
...
int kgsl_pwrscale_init(struct device *dev, const char *governor) {
  ...
  profile->initial_freq = pwr->pwrlevels[pwr->default_pwrlevel].gpu_freq;
  ...
  if (pwrscale->ctxt_aware_enable) {
    ...
    data->bin.ctxt_aware_target_pwrlevel = pwrscale->ctxt_aware_target_pwrlevel;
    data->bin.ctxt_aware_busy_penalty = pwrscale->ctxt_aware_busy_penalty;
  }
  ...
  if (of_property_read_bool(device->pdev->dev.of_node,
    ...
    if (kgsl_midframe) {
      hrtimer_init(&kgsl_midframe->timer, CLOCK_MONOTONIC, HRTIMER_MODE_REL);
      kgsl_midframe->timer.function = kgsl_pwrscale_midframe_timer;
      kgsl_midframe->device = device;
    } else
      KGSL_PWR_ERR(device, "Failed to enable-midframe-timer feature\n");
  }
  ...
  if (pwr->bus_control) {
    ...
    while (pwr->bus_ib[out] && out <= pwr->pwrlevels[0].bus_max) {
      pwr->bus_ib[out] = pwr->bus_ib[out] >> 20;
      out++;
    }
    data->bus.num = out;
  ...
  devfreq = devfreq_add_device(dev, &pwrscale->gpu_profile.profile, governor,
  ...
  pwrscale->cooling_dev =
      of_devfreq_cooling_register(device->pdev->dev.of_node, devfreq);
  ...
  if (data->bus.num) {
    ...
    pwrscale->bus_profile.profile.freq_table = pwrscale->freq_table;

    bus_devfreq = devfreq_add_device(
        device->busmondev, &pwrscale->bus_profile.profile, "gpubw_mon", NULL);
    if (!IS_ERR(bus_devfreq))
      pwrscale->gpu_profile.bus_devfreq = bus_devfreq;
  }

  ret = sysfs_create_link(&device->dev->kobj, &devfreq->dev.kobj, "devfreq");
  ...
  pwrscale->next_governor_call =
      ktime_add_us(ktime_get(), KGSL_GOVERNOR_CALL_INTERVAL);

  /* history tracking */
  for (i = 0; i < KGSL_PWREVENT_MAX; i++) {
    pwrscale->history[i].events = kzalloc(
        pwrscale->history[i].size * sizeof(struct kgsl_pwr_event), GFP_KERNEL);
...
static void do_devfreq_suspend(struct work_struct *work) {
  struct kgsl_pwrscale *pwrscale =
      container_of(work, struct kgsl_pwrscale, devfreq_suspend_ws);
...
static void do_devfreq_resume(struct work_struct *work) {
  struct kgsl_pwrscale *pwrscale =
      container_of(work, struct kgsl_pwrscale, devfreq_resume_ws);
  struct devfreq *devfreq = pwrscale->devfreqptr;
...
static void do_devfreq_notify(struct work_struct *work) {
  struct kgsl_pwrscale *pwrscale =
      container_of(work, struct kgsl_pwrscale, devfreq_notify_ws);
  struct devfreq *devfreq = pwrscale->devfreqptr;

  srcu_notifier_call_chain(&pwrscale->nh, ADRENO_DEVFREQ_NOTIFY_RETIRE,
                           devfreq);
}
