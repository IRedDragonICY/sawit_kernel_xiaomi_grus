...
/*
 * BACKPORT:[Cursor] KGSL Idle Opt - Reduce governor polling
 * interval from 10ms to 5ms. This allows the devfreq governor
 * to evaluate GPU load twice as often, enabling faster
 * frequency ramp-down when the GPU becomes idle.
 */
...
#define KGSL_PWRSCALE_INIT(_priv_data)                                         \
  {                                                                            \
      .enabled = true,                                                         \
      .gpu_profile = {.private_data = _priv_data,                              \
                      .profile =                                               \
                          {                                                    \
      ...
      .bus_profile = {.private_data = _priv_data,                              \
                      .profile =                                               \
                          {                                                    \
