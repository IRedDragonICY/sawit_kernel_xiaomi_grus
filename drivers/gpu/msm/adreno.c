...
static unsigned int counter_delta(struct kgsl_device *device, unsigned int reg,
...
static struct adreno_device device_3d0 = {
    ...
    .long_ib_detect = 1,
    .input_work = __WORK_INITIALIZER(device_3d0.input_work, adreno_input_work),
...
static unsigned int adreno_ft_regs_default[] = {
    ADRENO_REG_RBBM_STATUS,  ADRENO_REG_CP_RB_RPTR,  ADRENO_REG_CP_IB1_BASE,
    ADRENO_REG_CP_IB1_BUFSZ, ADRENO_REG_CP_IB2_BASE, ADRENO_REG_CP_IB2_BUFSZ};
 ...
 */
void adreno_readreg64(struct adreno_device *adreno_dev, enum adreno_regs lo,
                      enum adreno_regs hi, uint64_t *val) {
 ...
 */
void adreno_writereg64(struct adreno_device *adreno_dev, enum adreno_regs lo,
                       enum adreno_regs hi, uint64_t val) {
  struct adreno_gpudev *gpudev = ADRENO_GPU_DEVICE(adreno_dev);

  if (adreno_checkreg_off(adreno_dev, lo))
    kgsl_regwrite(KGSL_DEVICE(adreno_dev), gpudev->reg_offsets->offsets[lo],
                  lower_32_bits(val));
  if (adreno_checkreg_off(adreno_dev, hi))
    kgsl_regwrite(KGSL_DEVICE(adreno_dev), gpudev->reg_offsets->offsets[hi],
                  upper_32_bits(val));
}
...
unsigned int adreno_get_rptr(struct adreno_ringbuffer *rb) {
  ...
  if (adreno_is_a3xx(adreno_dev))
    ...
    kgsl_sharedmem_readl(&device->scratch, &rptr, SCRATCH_RPTR_OFFSET(rb->id));
  }
...
void adreno_efuse_speed_bin_array(struct adreno_device *adreno_dev) {
  ...
  if (bin_vector == NULL) {
    KGSL_DRV_ERR(device, "Unable to allocate memory for speed-bin vector\n");
    return;
  }

  if (of_property_read_u32_array(device->pdev->dev.of_node,
                                 "qcom,gpu-speed-bin-vectors", bin_vector,
                                 count)) {
    KGSL_DRV_ERR(device, "Speed-bin-vectors is invalid\n");
    kfree(bin_vector);
    return;
  }
  ...
  while (i < count) {
    ...
    adreno_dev->speed_bin |= (val & bin_vector[i + 1]) >> bin_vector[i + 2];
    i += vector_size;
  }
...
static int _get_counter(struct adreno_device *adreno_dev, int group,
                        int countable, unsigned int *lo, unsigned int *hi) {
  int ret = 0;

  if (*lo == 0) {

    ret = adreno_perfcounter_get(adreno_dev, group, countable, lo, hi,
...
static inline void _put_counter(struct adreno_device *adreno_dev, int group,
                                int countable, unsigned int *lo,
                                unsigned int *hi) {
  if (*lo != 0)
...
static void adreno_input_work(struct work_struct *work) {
  struct adreno_device *adreno_dev =
      container_of(work, struct adreno_device, input_work);
  struct kgsl_device *device = KGSL_DEVICE(adreno_dev);
...
static void adreno_input_event(struct input_handle *handle, unsigned int type,
  ...
  if (device->state == KGSL_STATE_NAP) {
    ...
    mod_timer(&device->idle_timer, jiffies + device->pwrctrl.interval_timeout);
...
static int _soft_reset(struct adreno_device *adreno_dev) {
  ...
  if (adreno_is_a530v1(adreno_dev)) {
    adreno_writereg(adreno_dev, ADRENO_REG_RBBM_BLOCK_SW_RESET_CMD, 0xFFDFFC0);
...
static irqreturn_t adreno_irq_handler(struct kgsl_device *device) {
  ...
  if (kgsl_gmu_isenabled(device)) {
    ...
    while (fence != 0) {
      ...
      if (fence_retries == FENCE_RETRY_MAX && fence != 0) {
        adreno_readreg(adreno_dev, ADRENO_REG_GMU_RBBM_INT_UNMASKED_STATUS,
                       &shadow_status);

        KGSL_DRV_CRIT_RATELIMIT(
            device, "AHB fence stuck in ISR: Shadow INT status=%8.8X\n",
  ...
  for (tmp = status; tmp != 0;) {
    ...
    if (irq_params->funcs[i].func != NULL) {
      ...
      KGSL_DRV_CRIT_RATELIMIT(device, "Unhandled interrupt bit %x\n", i);
   ...
   */
  if (status & int_bit)
    adreno_writereg(adreno_dev, ADRENO_REG_RBBM_INT_CLEAR_CMD, int_bit);
...
static struct {
  ...
  const char *prop;
    ...
    {ADRENO_QUIRK_DISABLE_LMLOADKILL, "qcom,gpu-quirk-lmloadkill-disable"},
    {ADRENO_QUIRK_HFI_USE_REG, "qcom,gpu-quirk-hfi-use-reg"},
    {ADRENO_QUIRK_SECVID_SET_ONCE, "qcom,gpu-quirk-secvid-set-once"},
    {ADRENO_QUIRK_LIMIT_UCHE_GBIF_RW, "qcom,gpu-quirk-limit-uche-gbif-rw"},
...
adreno_update_soc_hw_revision_quirks(struct adreno_device *adreno_dev,
  ...
  if (of_property_read_u32(node, "qcom,chipid", &adreno_dev->chipid))
    if (of_property_read_u32(pdev->dev.of_node, "qcom,chipid",
                             &adreno_dev->chipid))
      KGSL_DRV_FATAL(KGSL_DEVICE(adreno_dev), "No GPU chip ID was specified\n");
...
static void adreno_identify_gpu(struct adreno_device *adreno_dev) {
  ...
  if (adreno_dev->gpucore == NULL)
    KGSL_DRV_FATAL(KGSL_DEVICE(adreno_dev), "Unknown GPU chip ID %8.8X\n",
                   adreno_dev->chipid);
...
static void adreno_of_get_ca_target_pwrlevel(struct adreno_device *adreno_dev,
  ...
  of_property_read_u32(node, "qcom,ca-target-pwrlevel", &ca_target_pwrlevel);
  ...
  if (pwrscale->ctxt_aware_enable) {
    ...
    for_each_child_of_node(node, child) {
      ...
      if (bin == adreno_dev->speed_bin) {
        adreno_of_get_ca_target_pwrlevel(adreno_dev, child);
        return;
      }
    }
...
static int adreno_of_parse_pwrlevels(struct adreno_device *adreno_dev,
  ...
  for_each_child_of_node(node, child) {
    ...
    if (adreno_of_read_property(child, "qcom,gpu-freq", &level->gpu_freq))
      return -EINVAL;

    if (adreno_of_read_property(child, "qcom,bus-freq", &level->bus_freq))
      return -EINVAL;

    if (of_property_read_u32(child, "qcom,bus-min", &level->bus_min))
      level->bus_min = level->bus_freq;

    if (of_property_read_u32(child, "qcom,bus-max", &level->bus_max))
...
static void l3_pwrlevel_probe(struct kgsl_device *device,
                              struct device_node *node) {
  ...
  for_each_child_of_node(pwrlevel_node, child) {
    ...
    if (of_property_read_u32(child, "qcom,l3-freq", &device->l3_freq[index]))
      return;
  }
...
static inline struct adreno_device *
adreno_get_dev(struct platform_device *pdev) {
...
static int adreno_of_get_power(struct adreno_device *adreno_dev,
  ...
  if (of_property_read_u32(node, "qcom,idle-timeout", &timeout))
    /*
     * BACKPORT:[Cursor] KGSL Idle Opt - Reduce default
     * idle timeout from 80ms to 40ms. This triggers
     * power collapse sooner during idle gaps, reducing
     * sustained GPU idle power. 40ms is ~2.5 frames at
     * 60fps, still safe for bursty workloads.
     */
    timeout = 40;

  device->pwrctrl.interval_timeout = msecs_to_jiffies(timeout);

  device->pwrctrl.bus_control = of_property_read_bool(node, "qcom,bus-control");

  device->pwrctrl.input_disable =
      of_property_read_bool(node, "qcom,disable-wake-on-touch");

  return 0;
}

#ifdef CONFIG_QCOM_OCMEM
static int adreno_ocmem_malloc(struct adreno_device *adreno_dev) {
...
static void adreno_ocmem_free(struct adreno_device *adreno_dev) {
  if (adreno_dev->ocmem_hdl != NULL) {
...
#else
static int adreno_ocmem_malloc(struct adreno_device *adreno_dev) { return 0; }
...
static void adreno_cx_dbgc_probe(struct kgsl_device *device) {
  ...
  adreno_dev->cx_dbgc_virt =
      devm_ioremap(device->dev, device->reg_phys + adreno_dev->cx_dbgc_base,
...
static void adreno_cx_misc_probe(struct kgsl_device *device) {
  ...
  adreno_dev->cx_misc_virt =
      devm_ioremap(device->dev, res->start, adreno_dev->cx_misc_len);
}
...
static void adreno_efuse_read_soc_hw_rev(struct adreno_device *adreno_dev) {
  ...
  if (of_property_read_u32_array(KGSL_DEVICE(adreno_dev)->pdev->dev.of_node,
                                 "qcom,soc-hw-rev-efuse", soc_hw_rev, 3))
    return;

  ret = adreno_efuse_map(adreno_dev);
  if (ret) {
    KGSL_CORE_ERR("Unable to map hardware revision fuse: ret=%d\n", ret);
    return;
  }
  ...
  if (ret) {
    KGSL_CORE_ERR("Unable to read hardware revision fuse: ret=%d\n", ret);
    return;
  }
...
static bool adreno_is_gpu_disabled(struct adreno_device *adreno_dev) {
  ...
  return (row0 >> pte_row0_msb[2]) & pte_row0_msb[1] ? true : false;
}
...
static int adreno_probe(struct platform_device *pdev) {
  ...
  if (IS_ERR(adreno_dev->gpu_llc_slice)) {
    KGSL_DRV_WARN(device, "Failed to get GPU LLC slice descriptor (%ld)\n",
  ...
  if (IS_ERR(adreno_dev->gpuhtw_llc_slice)) {
    KGSL_DRV_WARN(device, "Failed to get gpuhtw LLC slice descriptor (%ld)\n",
...
static void adreno_fault_detect_init(struct adreno_device *adreno_dev) {
  ...
  if (!(swfdetect || ADRENO_FEATURE(adreno_dev, ADRENO_SOFT_FAULT_DETECT)))
    return;
  ...
  adreno_ft_regs_num =
      (ARRAY_SIZE(adreno_ft_regs_default) + gpudev->ft_perf_counters_count * 2);

  adreno_ft_regs =
      kcalloc(adreno_ft_regs_num, sizeof(unsigned int), GFP_KERNEL);
  adreno_ft_regs_val =
      kcalloc(adreno_ft_regs_num, sizeof(unsigned int), GFP_KERNEL);
  ...
  for (i = 0; i < ARRAY_SIZE(adreno_ft_regs_default); i++)
    adreno_ft_regs[i] = adreno_getreg(adreno_dev, adreno_ft_regs_default[i]);
...
static int adreno_init(struct kgsl_device *device) {
  ...
  if (!adreno_is_a3xx(adreno_dev))
    kgsl_sharedmem_set(device, &device->scratch, 0, 0, device->scratch.size);
  ...
  if (adreno_dev->gpucore->shader_offset && adreno_dev->gpucore->shader_size) {

    if (device->shader_mem_phys || device->shader_mem_virt)
      KGSL_DRV_ERR(device, "Shader memory already specified in device tree\n");
    else {
      device->shader_mem_phys =
          device->reg_phys + adreno_dev->gpucore->shader_offset;
      device->shader_mem_virt =
          device->reg_virt + adreno_dev->gpucore->shader_offset;
      device->shader_mem_len = adreno_dev->gpucore->shader_size;
    }
  }
  ...
  if (!adreno_is_a3xx(adreno_dev)) {
    int r = kgsl_allocate_global(device, &adreno_dev->profile_buffer, PAGE_SIZE,
                                 0, 0, "alwayson");
    ...
    if (r == 0) {
      ...
      kgsl_sharedmem_set(device, &adreno_dev->profile_buffer, 0, 0, PAGE_SIZE);
    }
  }
...
static bool regulators_left_on(struct kgsl_device *device) {
  ...
  for (i = 0; i < KGSL_MAX_REGULATORS; i++) {
    struct kgsl_regulator *regulator = &device->pwrctrl.regulators[i];
...
static void _set_secvid(struct kgsl_device *device) {
  ...
  if (device->mmu.secured && !set) {
    if (adreno_is_a4xx(adreno_dev))
      adreno_writereg(adreno_dev, ADRENO_REG_RBBM_SECVID_TRUST_CONFIG, 0x2);
    adreno_writereg(adreno_dev, ADRENO_REG_RBBM_SECVID_TSB_CONTROL, 0x0);

    adreno_writereg64(adreno_dev, ADRENO_REG_RBBM_SECVID_TSB_TRUSTED_BASE,
...
static int adreno_switch_to_unsecure_mode(struct adreno_device *adreno_dev,
  ...
  if (ret)
    adreno_spin_idle_debug(adreno_dev, "Switch to unsecure failed to idle\n");
...
int adreno_set_unsecured_mode(struct adreno_device *adreno_dev,
  ...
  if (adreno_dev->zap_loaded)
    ...
    adreno_writereg(adreno_dev, ADRENO_REG_RBBM_SECVID_TRUST_CONTROL, 0x0);
...
static void adreno_set_active_ctxs_null(struct adreno_device *adreno_dev) {
  ...
  FOR_EACH_RINGBUFFER(adreno_dev, rb, i) {
    ...
    kgsl_sharedmem_writel(KGSL_DEVICE(adreno_dev), &rb->pagetable_desc,
...
static int _adreno_start(struct adreno_device *adreno_dev) {
  ...
  kgsl_pwrctrl_update_l2pc(&adreno_dev->dev, KGSL_L2PC_WAKEUP_TIMEOUT);

  pm_qos_update_request(&device->pwrctrl.pm_qos_req_dma, pmqos_wakeup_vote);
  ...
  if (gpudev->oob_set) {
    status = gpudev->oob_set(adreno_dev, OOB_GPU_SET_MASK, OOB_GPU_CHECK_MASK,
                             OOB_GPU_CLEAR_MASK);
    if (status)
      goto error_mmu_off;
  }
  ...
  if (gpudev->enable_64bit && adreno_support_64bit(adreno_dev))
  ...
  if (device->pwrctrl.bus_control) {
    ...
    if (adreno_dev->starved_ram_lo == 0) {
      ret = adreno_perfcounter_get(adreno_dev, KGSL_PERFCOUNTER_GROUP_VBIF_PWR,
                                   0, &adreno_dev->starved_ram_lo, NULL,
                                   PERFCOUNTER_FLAG_KERNEL);

      if (ret) {
        KGSL_DRV_ERR(device, "Unable to get perf counters for bus DCVS\n");
    ...
    if (adreno_has_gbif(adreno_dev)) {
      if (adreno_dev->starved_ram_lo_ch1 == 0) {
        ret = adreno_perfcounter_get(
            adreno_dev, KGSL_PERFCOUNTER_GROUP_VBIF_PWR, 1,
            &adreno_dev->starved_ram_lo_ch1, NULL, PERFCOUNTER_FLAG_KERNEL);
      ...
      if (adreno_dev->ram_cycles_lo == 0) {
        ret = adreno_perfcounter_get(
            adreno_dev, KGSL_PERFCOUNTER_GROUP_VBIF, VBIF_AXI_TOTAL_BEATS,
            &adreno_dev->ram_cycles_lo, NULL, PERFCOUNTER_FLAG_KERNEL);
  ...
  if (pmqos_active_vote != pmqos_wakeup_vote)
    pm_qos_update_request(&device->pwrctrl.pm_qos_req_dma, pmqos_active_vote);
  ...
  if (gpudev->oob_clear) {
    ...
    if (ADRENO_QUIRK(adreno_dev, ADRENO_QUIRK_HFI_USE_REG))
      gpudev->oob_clear(adreno_dev, OOB_BOOT_SLUMBER_CLEAR_MASK);
  }
...
error_boot_oob_clear:
  if (gpudev->oob_clear && ADRENO_QUIRK(adreno_dev, ADRENO_QUIRK_HFI_USE_REG))
    gpudev->oob_clear(adreno_dev, OOB_BOOT_SLUMBER_CLEAR_MASK);
...
static int adreno_stop(struct kgsl_device *device) {
  ...
  if (gpudev->oob_set) {
    error = gpudev->oob_set(adreno_dev, OOB_GPU_SET_MASK, OOB_GPU_CHECK_MASK,
                            OOB_GPU_CLEAR_MASK);
    if (error) {
      struct gmu_device *gmu = &device->gmu;

      gpudev->oob_clear(adreno_dev, OOB_GPU_CLEAR_MASK);
      if (gmu->gx_gdsc && regulator_is_enabled(gmu->gx_gdsc)) {
...
static inline bool adreno_try_soft_reset(struct kgsl_device *device,
                                         int fault) {
  struct adreno_device *adreno_dev = ADRENO_DEVICE(device);
...
static int adreno_getproperty(struct kgsl_device *device, unsigned int type,
  ...
  case KGSL_PROP_DEVICE_INFO: {
    ...
    devinfo.mmu_enabled = MMU_FEATURE(&device->mmu, KGSL_MMU_PAGED);
    devinfo.gmem_gpubaseaddr = adreno_dev->gmem_base;
    devinfo.gmem_sizebytes = adreno_dev->gmem_size;

    if (copy_to_user(value, &devinfo, sizeof(devinfo)) != 0) {
      status = -EFAULT;
      break;
    }
    status = 0;
  ...
  case KGSL_PROP_DEVICE_SHADOW: {
    ...
    if (device->memstore.hostptr) {
       ...
       */
      shadowprop.flags =
          KGSL_FLAGS_INITIALIZED | KGSL_FLAGS_PER_CONTEXT_TIMESTAMPS;
    }
    if (copy_to_user(value, &shadowprop, sizeof(shadowprop))) {
      status = -EFAULT;
      break;
    }
    status = 0;
  ...
  case KGSL_PROP_DEVICE_QDSS_STM: {
    ...
    if (copy_to_user(value, &qdssprop, sizeof(qdssprop))) {
      status = -EFAULT;
      break;
    }
    status = 0;
  ...
  case KGSL_PROP_DEVICE_QTIMER: {
    ...
    struct kgsl_memdesc *qtimer_desc = kgsl_mmu_get_qtimer_global_entry(device);
    ...
    if (copy_to_user(value, &qtimerprop, sizeof(qtimerprop))) {
      status = -EFAULT;
      break;
    }
    status = 0;
  ...
  case KGSL_PROP_MMU_ENABLE: {
    ...
    int mmu_prop = MMU_FEATURE(&device->mmu, KGSL_MMU_PAGED);
  ...
  case KGSL_PROP_UCHE_GMEM_VADDR: {
    ...
    if (copy_to_user(value, &gmem_vaddr, sizeof(uint64_t))) {
      status = -EFAULT;
      break;
    }
    status = 0;
  ...
  case KGSL_PROP_HIGHEST_BANK_BIT: {
    ...
    if (of_property_read_u32(device->pdev->dev.of_node, "qcom,highest-bank-bit",
                             &bit)) {
      status = -EINVAL;
      break;
    }
  ...
  case KGSL_PROP_UBWC_MODE: {
    ...
    if (of_property_read_u32(device->pdev->dev.of_node, "qcom,ubwc-mode",
                             &mode))
      mode = 0;
  ...
  case KGSL_PROP_DEVICE_BITNESS: {
    ...
    if (copy_to_user(value, &bitness, sizeof(unsigned int))) {
      status = -EFAULT;
      break;
    }
    status = 0;
...
int adreno_set_constraint(struct kgsl_device *device,
  ...
  case KGSL_CONSTRAINT_PWRLEVEL: {
    ...
    if (copy_from_user(&pwr, (void __user *)constraint->data, sizeof(pwr))) {
      status = -EFAULT;
      break;
    }
    ...
    context->pwr_constraint.type = KGSL_CONSTRAINT_PWRLEVEL;
  ...
  if ((status == 0) && (context->id == device->pwrctrl.constraint.owner_id)) {
...
static int adreno_setproperty(struct kgsl_device_private *dev_priv,
  ...
  case KGSL_PROP_PWR_CONSTRAINT: {
    ...
    if (copy_from_user(&constraint, value, sizeof(constraint))) {
      status = -EFAULT;
      break;
    }

    context = kgsl_context_get_owner(dev_priv, constraint.context_id);

    if (context == NULL)
      break;

    status = adreno_set_constraint(device, context, &constraint);
...
int adreno_soft_reset(struct kgsl_device *device) {
  ...
  if (gpudev->oob_set) {
    ret = gpudev->oob_set(adreno_dev, OOB_GPU_SET_MASK, OOB_GPU_CHECK_MASK,
                          OOB_GPU_CLEAR_MASK);
    if (ret)
      return ret;
  }
...
static void adreno_regread(struct kgsl_device *device, unsigned int offsetwords,
                           unsigned int *value) {
  adreno_read(device, device->reg_virt, offsetwords, value, device->reg_len);
}
...
bool adreno_is_cx_dbgc_register(struct kgsl_device *device,
  ...
  return adreno_dev->cx_dbgc_virt &&
         ...
         (offsetwords < (adreno_dev->cx_dbgc_base + adreno_dev->cx_dbgc_len) >>
          2);
}
...
void adreno_cx_misc_regread(struct adreno_device *adreno_dev,
  ...
  if (!adreno_dev->cx_misc_virt || (cx_misc_offset >= adreno_dev->cx_misc_len))
    return;
...
static int adreno_waittimestamp(struct kgsl_device *device,
  ...
  ret = adreno_drawctxt_wait(ADRENO_DEVICE(device), context, timestamp, msecs);
 ...
 */
int adreno_rb_readtimestamp(struct adreno_device *adreno_dev, void *priv,
  ...
  if (type == KGSL_TIMESTAMP_QUEUED)
    *timestamp = rb->timestamp;
    ...
    status = __adreno_readtimestamp(adreno_dev, rb->id + KGSL_MEMSTORE_MAX,
                                    type, timestamp);
 ...
 * address and returns -EINVAL on any read error/invalid type and timestamp = 0.
 */
static int adreno_readtimestamp(struct kgsl_device *device, void *priv,
                                enum kgsl_timestamp_type type,
                                unsigned int *timestamp) {
  int status = 0;
  ...
  if (type == KGSL_TIMESTAMP_QUEUED) {
    ...
    *timestamp = ctxt->timestamp;
    ...
    status = __adreno_readtimestamp(ADRENO_DEVICE(device), context->id, type,
                                    timestamp);
 ...
 */
static void
adreno_device_private_destroy(struct kgsl_device_private *dev_priv) {
  struct kgsl_device *device = dev_priv->device;
  struct adreno_device *adreno_dev = ADRENO_DEVICE(device);
  struct adreno_device_private *adreno_priv =
      container_of(dev_priv, struct adreno_device_private, dev_priv);
  struct adreno_perfcounter_list_node *p, *tmp;

  mutex_lock(&device->mutex);
  list_for_each_entry_safe(p, tmp, &adreno_priv->perfcounter_list, node) {
    adreno_perfcounter_put(adreno_dev, p->groupid, p->countable,
...
static void adreno_power_stats(struct kgsl_device *device,
  ...
  if (adreno_dev->perfctr_pwr_lo != 0) {
    ...
    gpu_busy =
        counter_delta(device, adreno_dev->perfctr_pwr_lo, &busy->gpu_busy);
    ...
    if (adreno_is_a6xx(adreno_dev)) {
      ...
      stats->busy_time =
          adreno_ticks_to_us(gpu_busy, kgsl_pwrctrl_active_freq(pwr));
    }
  }
  ...
  if (device->pwrctrl.bus_control) {
    ...
    if (adreno_has_gbif(adreno_dev)) {
      if (adreno_dev->ram_cycles_lo_ch1_read != 0)
        ram_cycles += counter_delta(device, adreno_dev->ram_cycles_lo_ch1_read,
                                    &busy->bif_ram_cycles_read_ch1);

      if (adreno_dev->ram_cycles_lo_ch0_write != 0)
        ram_cycles += counter_delta(device, adreno_dev->ram_cycles_lo_ch0_write,
                                    &busy->bif_ram_cycles_write_ch0);

      if (adreno_dev->ram_cycles_lo_ch1_write != 0)
        ram_cycles += counter_delta(device, adreno_dev->ram_cycles_lo_ch1_write,
    ...
    if (adreno_has_gbif(adreno_dev)) {
      if (adreno_dev->starved_ram_lo_ch1 != 0)
        starved_ram += counter_delta(device, adreno_dev->starved_ram_lo_ch1,
  ...
  if (adreno_dev->perfctr_ifpc_lo != 0) {
    ...
    num_ifpc =
        counter_delta(device, adreno_dev->perfctr_ifpc_lo, &busy->num_ifpc);
    adreno_dev->ifpc_count += num_ifpc;
    if (num_ifpc > 0)
      trace_adreno_ifpc_count(adreno_dev->ifpc_count);
  }

  if (adreno_dev->lm_threshold_count && gpudev->count_throttles)
...
static int adreno_regulator_enable(struct kgsl_device *device) {
  ...
  if (gpudev->regulator_enable &&
      !test_bit(ADRENO_DEVICE_GPU_REGULATOR_ENABLED, &adreno_dev->priv)) {
...
static bool adreno_is_hw_collapsible(struct kgsl_device *device) {
  ...
  return adreno_isidle(device) &&
         (gpudev->is_sptp_idle ? gpudev->is_sptp_idle(adreno_dev) : true);
}
...
static void adreno_pwrlevel_change_settings(struct kgsl_device *device,
                                            ...
                                            unsigned int postlevel, bool post) {
  struct adreno_device *adreno_dev = ADRENO_DEVICE(device);
  struct adreno_gpudev *gpudev = ADRENO_GPU_DEVICE(adreno_dev);

  if (gpudev->pwrlevel_change_settings)
    gpudev->pwrlevel_change_settings(adreno_dev, prelevel, postlevel, post);
}
...
static void adreno_clk_set_options(struct kgsl_device *device, const char *name,
  ...
  if (ADRENO_GPU_DEVICE(ADRENO_DEVICE(device))->clk_set_options)
    ADRENO_GPU_DEVICE(ADRENO_DEVICE(device))
        ->clk_set_options(ADRENO_DEVICE(device), name, clk, on);
}
...
static void adreno_iommu_sync(struct kgsl_device *device, bool sync) {
  ...
  if (sync == true) {
    ...
    if (ret)
      KGSL_DRV_ERR(device, "MMU sync with Hypervisor off %x\n", ret);
  } else {
    desc.args[0] = false;
    desc.arginfo = SCM_ARGS(1);
