...
#define SUSFS_LOGI(fmt, ...)                                                   \
  if (is_log_enable)                                                           \
  pr_info("susfs:[%u][%u][%s] " fmt, current_uid().val, current->pid,          \
          __func__, ##__VA_ARGS__)
#define SUSFS_LOGE(fmt, ...)                                                   \
  if (is_log_enable)                                                           \
  pr_err("susfs:[%u][%u][%s]" fmt, current_uid().val, current->pid, __func__,  \
         ##__VA_ARGS__)
#else
#define SUSFS_LOGI(fmt, ...)
#define SUSFS_LOGE(fmt, ...)
...
int susfs_add_sus_kstat(struct st_susfs_sus_kstat *__user user_info) {
  ...
  list_for_each_entry_safe(cursor, temp, &LH_SUS_KSTAT_SPOOFER, list) {
    if (cursor->info.target_ino == info.target_ino) {
      if (info.target_pathname[0] != '\0') {
        SUSFS_LOGE("target_pathname: '%s' is already created in "
                   "LH_SUS_KSTAT_SPOOFER\n",
                   info.target_pathname);
      } else {
        SUSFS_LOGE(
            "target_ino: '%lu' is already created in LH_SUS_KSTAT_SPOOFER\n",
            info.target_ino);
      }
      return 1;
    }
  ...
  /* Seems the dev number issue is finally solved, the userspace stat we see is
   * already a encoded dev which is set by new_encode_dev() / huge_encode_dev()
   * function for 64bit system and old_encode_dev() for 32bit only system,
   * that's why we need to decode it in kernel as well, and different kernel may
   * have different function to encode the dev number, be cautious! Also check
   * your encode_dev() macro in fs/stat.c to determine which one to use
   */
  ...
  spin_unlock(&susfs_spin_lock);
  SUSFS_LOGI(
      "target_ino: '%lu', target_pathname: '%s', spoofed_pathname: '%s', "
      "spoofed_ino: '%lu', spoofed_dev: '%lu', spoofed_nlink: '%u', "
      "spoofed_atime_tv_sec: '%ld', spoofed_mtime_tv_sec: '%ld', "
      "spoofed_ctime_tv_sec: '%ld', spoofed_atime_tv_nsec: '%ld', "
      "spoofed_mtime_tv_nsec: '%ld', spoofed_ctime_tv_nsec: '%ld', is "
      "successfully added to LH_SUS_KSTAT_SPOOFER\n",
      new_list->info.target_ino, new_list->info.target_pathname,
      new_list->info.spoofed_pathname, new_list->info.spoofed_ino,
      new_list->info.spoofed_dev, new_list->info.spoofed_nlink,
      new_list->info.spoofed_atime_tv_sec, new_list->info.spoofed_mtime_tv_sec,
      new_list->info.spoofed_ctime_tv_sec, new_list->info.spoofed_atime_tv_nsec,
      new_list->info.spoofed_mtime_tv_nsec,
      new_list->info.spoofed_ctime_tv_nsec);
  return 0;
}
...
int susfs_update_sus_kstat(struct st_susfs_sus_kstat *__user user_info) {
  ...
  list_for_each_entry_safe(cursor, temp, &LH_SUS_KSTAT_SPOOFER, list) {
    if (unlikely(!strcmp(info.target_pathname, cursor->info.target_pathname))) {
      SUSFS_LOGI("updating target_ino from '%lu' to '%lu' for pathname: '%s' "
                 "in LH_SUS_KSTAT_SPOOFER\n",
...
int susfs_add_sus_maps(struct st_susfs_sus_maps *__user user_info) {
  ...
  list_for_each_entry_safe(cursor, temp, &LH_SUS_MAPS_SPOOFER, list) {
    if (cursor->info.is_statically == info.is_statically &&
        !info.is_statically) {
      if (cursor->info.target_ino == info.target_ino) {
        SUSFS_LOGE("is_statically: '%d', target_ino: '%lu', is already created "
                   "in LH_SUS_MAPS_SPOOFER\n",
                   info.is_statically, info.target_ino);
        return 1;
      }
    } else if (cursor->info.is_statically == info.is_statically &&
               info.is_statically) {
      if (cursor->info.compare_mode == info.compare_mode &&
          info.compare_mode == 1) {
        if (cursor->info.target_ino == info.target_ino) {
          SUSFS_LOGE("is_statically: '%d', compare_mode: '%d', target_ino: "
                     "'%lu', is already created in LH_SUS_MAPS_SPOOFER\n",
                     info.is_statically, info.compare_mode, info.target_ino);
          return 1;
        }
      } else if (cursor->info.compare_mode == info.compare_mode &&
                 info.compare_mode == 2) {
        if (cursor->info.target_ino == info.target_ino &&
          ...
          SUSFS_LOGE(
              "is_statically: '%d', compare_mode: '%d', target_ino: '%lu', "
              "is_isolated_entry: '%d', target_pgoff: '0x%x', target_prot: "
              "'0x%x', is already created in LH_SUS_MAPS_SPOOFER\n",
              info.is_statically, info.compare_mode, info.target_ino,
        ...
        if (info.target_ino == 0 &&
          ...
          SUSFS_LOGE("is_statically: '%d', compare_mode: '%d', target_ino: "
                     "'%lu', prev_target_ino: '%lu', next_target_ino: '%lu', "
                     "is already created in LH_SUS_MAPS_SPOOFER\n",
                     info.is_statically, info.compare_mode, info.target_ino,
        ...
        if (cursor->info.is_file == info.is_file &&
            ...
            cursor->info.target_prot == info.target_prot &&
            cursor->info.target_addr_size == info.target_addr_size) {
          SUSFS_LOGE("is_statically: '%d', compare_mode: '%d', is_file: '%d', "
                     "target_dev: '0x%x', target_pgoff: '0x%x', target_prot: "
                     "'0x%x', target_addr_size: '0x%x', is already created in "
                     "LH_SUS_MAPS_SPOOFER\n",
  ...
  SUSFS_LOGI(
      "is_statically: '%d', compare_mode: '%d', is_isolated_entry: '%d', "
      "is_file: '%d', prev_target_ino: '%lu', next_target_ino: '%lu', "
      "target_ino: '%lu', target_dev: '0x%x', target_pgoff: '0x%x', "
      "target_prot: '0x%x', target_addr_size: '0x%x', spoofed_pathname: '%s', "
      "spoofed_ino: '%lu', spoofed_dev: '0x%x', spoofed_pgoff: '0x%x', "
      "spoofed_prot: '0x%x', is successfully added to LH_SUS_MAPS_SPOOFER\n",
...
int susfs_update_sus_maps(struct st_susfs_sus_maps *__user user_info) {
  ...
  list_for_each_entry_safe(cursor, temp, &LH_SUS_MAPS_SPOOFER, list) {
    if (cursor->info.is_statically == info.is_statically &&
        !info.is_statically) {
      if (unlikely(
              !strcmp(info.target_pathname, cursor->info.target_pathname))) {
        SUSFS_LOGI("updating target_ino from '%lu' to '%lu' for pathname: '%s' "
                   "in LH_SUS_MAPS_SPOOFER\n",
                   cursor->info.target_ino, info.target_ino,
                   info.target_pathname);
...
int susfs_add_sus_proc_fd_link(
    struct st_susfs_sus_proc_fd_link *__user user_info) {
  ...
  if (copy_from_user(&info, user_info,
  ...
  list_for_each_entry_safe(cursor, temp, &LH_SUS_PROC_FD_LINK, list) {
    if (unlikely(
            !strcmp(info.target_link_name, cursor->info.target_link_name))) {
      SUSFS_LOGE(
          "target_link_name: '%s' is already created in LH_SUS_PROC_FD_LINK\n",
  ...
  spin_unlock(&susfs_spin_lock);
  SUSFS_LOGI("target_link_name: '%s', spoofed_link_name: '%s', is successfully "
             "added to LH_SUS_PROC_FD_LINK\n",
...
int susfs_add_try_umount(struct st_susfs_try_umount *__user user_info) {
  ...
  list_for_each_entry_safe(cursor, temp, &LH_TRY_UMOUNT_PATH, list) {
    if (unlikely(!strcmp(info.target_pathname, cursor->info.target_pathname))) {
      SUSFS_LOGE(
          "target_pathname: '%s' is already created in LH_TRY_UMOUNT_PATH\n",
          info.target_pathname);
      return 1;
    }
  }
  ...
  spin_unlock(&susfs_spin_lock);
  SUSFS_LOGI("target_pathname: '%s', mnt_mode: %d, is successfully added to "
             "LH_TRY_UMOUNT_PATH\n",
...
int susfs_set_uname(struct st_susfs_uname *__user user_info) {
  ...
  SUSFS_LOGI("setting sysname: '%s', nodename: '%s', release: '%s', version: "
             "'%s', machine: '%s'\n",
...
int susfs_sus_path_by_path(struct path *file, int *errno_to_be_changed,
                           int syscall_family)
#else
int susfs_sus_path_by_path(const struct path *file, int *errno_to_be_changed,
                           int syscall_family)
#endif
{
  int res = 0;
  ...
  list_for_each_entry_safe(cursor, temp, &LH_SUS_PATH, list) {
    if (unlikely(!strcmp(cursor->info.target_pathname, path))) {
      ...
      if (errno_to_be_changed != NULL) {
        susfs_change_error_no_by_pathname(path, errno_to_be_changed,
                                          syscall_family);
      }
      status = 1;
      goto out_free_path;
...
int susfs_sus_path_by_filename(struct filename *name, int *errno_to_be_changed,
                               int syscall_family) {
  int status = 0;
  int ret = 0;
  struct path path;
 ...
 *  current process and save to a list of corresponding spoofed mnt_id and
 * parent_mnt_id once process with uid >= 10000 opens /proc/self/mountinfo
 */
void susfs_add_mnt_id_recorder(struct mnt_namespace *ns) {
  ...
  list_for_each_entry_safe(recorder_cursor, recorder_temp,
                           &LH_MOUNT_ID_RECORDER, list) {
    if (recorder_cursor->pid == cur_pid) {
      recorder_cursor->opened_count++;
      SUSFS_LOGI("mountinfo opened by the same pid: '%d', "
                 "recorder_cursor->opened_count: '%d'\n",
  ...
  new_recorder_list =
      kzalloc(sizeof(struct st_susfs_mnt_id_recorder_list), GFP_KERNEL);
  ...
  list_for_each_entry_safe(mnt_cursor, mnt_temp, &ns->list, mnt_list) {
    ...
    if (count == 0) {
      ...
      new_recorder_list->info.spoofed_parent_mnt_id[count] =
          mnt_cursor->mnt_parent->mnt_id;
    ...
    list_for_each_entry_safe(sus_mount_cursor, sus_mount_temp, &LH_SUS_MOUNT,
                             list) {
      // skip adding this mount to recorder list if it is suspicious
      if (unlikely(!strcmp(path, sus_mount_cursor->info.target_pathname))) {
        SUSFS_LOGI("skip adding target_mnt_id: '%d', target_pathname: '%s' to "
    ...
    new_recorder_list->info.target_mnt_id[count] = mnt_cursor->mnt_id;
    new_recorder_list->info.spoofed_mnt_id[count] =
        new_recorder_list->info.spoofed_mnt_id[0] + count;
    for (i = 0; i < count; i++) {
      if (mnt_cursor->mnt_parent->mnt_id ==
  ...
  for (i = 0; i<new_recorder_list->info.count; i++) {
          SUSFS_LOGI("target_mnt_id: %d, spoofed_mnt_id: %d,
  spoofed_parent_mnt_id: %d\n", new_recorder_list->info.target_mnt_id[i],
                          new_recorder_list->info.spoofed_mnt_id[i],
...
/* for non statically, it only compare with target_ino, and spoof only the ino,
 * dev to the matched entry for staticially, it compares depending on the mode
 * user chooses compare mode: 1 -> target_ino is 'non-zero', all entries match
 * with target_ino will be spoofed with user defined entry 2 -> target_ino is
 * 'non-zero', all entries match with
 * [target_ino,target_addr_size,target_prot,target_pgoff,is_isolated_entry] will
 * be spoofed with user defined entry 3 -> target_ino is 'zero', which is not
 * file, all entries match with [prev_target_ino,next_target_ino] will be
 * spoofed with user defined entry 4 -> target_ino is 'zero' or 'non-zero', all
 * entries match with
 * [is_file,target_addr_size,target_prot,target_pgoff,target_dev] will be
 * spoofed with user defined entry
 */
int susfs_sus_maps(unsigned long target_ino, unsigned long target_addr_size,
                   ...
                   unsigned long long *pgoff, struct vm_area_struct *vma,
                   char *out_name) {
  struct st_susfs_sus_maps_list *cursor, *temp;
    ...
    if (!cursor->info.is_statically) {
      if (target_ino != 0 && cursor->info.target_ino == target_ino) {
        ...
        *orig_dev = cursor->info.spoofed_dev;
        SUSFS_LOGI(
            "spoofing maps -> is_statically: '%d', compare_mode: '%d', "
            "is_file: '%d', is_isolated_entry: '%d', prev_target_ino: '%lu', "
            "next_target_ino: '%lu', target_ino: '%lu', target_dev: '0x%x', "
            "target_pgoff: '0x%x', target_prot: '0x%x', target_addr_size: "
            "'0x%x', spoofed_pathname: '%s', spoofed_ino: '%lu', spoofed_dev: "
            "'0x%x', spoofed_pgoff: '0x%x', spoofed_prot: '0x%x'\n",
            cursor->info.is_statically, cursor->info.compare_mode,
            cursor->info.is_file, cursor->info.is_isolated_entry,
            cursor->info.prev_target_ino, cursor->info.next_target_ino,
            cursor->info.target_ino, cursor->info.target_dev,
            cursor->info.target_pgoff, cursor->info.target_prot,
            cursor->info.target_addr_size, cursor->info.spoofed_pathname,
            cursor->info.spoofed_ino, cursor->info.spoofed_dev,
            cursor->info.spoofed_pgoff, cursor->info.spoofed_prot);
        return 1;
      }
      ...
      case 2:
        if (target_ino != 0 && cursor->info.target_ino == target_ino &&
            ...
            ((cursor->info.target_prot & VM_MAYSHARE) ==
             (*flags & VM_MAYSHARE)) &&
            cursor->info.target_addr_size == target_addr_size &&
            cursor->info.target_pgoff == *pgoff) {
          // if is NOT isolated_entry, check for vma->vm_next and vma->vm_prev
          // to see if they have the same inode
          if (!cursor->info.is_isolated_entry) {
            ...
            // both prev and next don't have the same indoe as current entry, we
            // can spoof now
            goto do_spoof;
          }
        }
      ...
      case 3:
        ...
        if (cursor->info.prev_target_ino == 0 &&
            cursor->info.next_target_ino > 0) {
        ...
        } else if (cursor->info.prev_target_ino > 0 &&
                   cursor->info.next_target_ino == 0) {
          ...
          // compare both prev ino and next ino
                   ...
                   cursor->info.next_target_ino > 0) {
          if (vma->vm_prev && vma->vm_prev->vm_file && vma->vm_next &&
              vma->vm_next->vm_file) {
            tmp_inode_prev = file_inode(vma->vm_prev->vm_file);
            tmp_inode_next = file_inode(vma->vm_next->vm_file);
            if (tmp_inode_prev->i_ino == cursor->info.prev_target_ino &&
        ...
        break;
      case 4:
        if ((cursor->info.is_file && vma->vm_file) ||
            (!cursor->info.is_file && !vma->vm_file)) {
  ...
  do_spoof:
    if (cursor->info.need_to_spoof_pathname) {
      strncpy(out_name, cursor->info.spoofed_pathname,
    ...
    if (cursor->info.need_to_spoof_prot) {
      if (cursor->info.spoofed_prot & VM_READ)
        *flags |= VM_READ;
      else
        *flags = ((*flags | VM_READ) ^ VM_READ);
      if (cursor->info.spoofed_prot & VM_WRITE)
        *flags |= VM_WRITE;
      else
        *flags = ((*flags | VM_WRITE) ^ VM_WRITE);
      if (cursor->info.spoofed_prot & VM_EXEC)
        *flags |= VM_EXEC;
      else
        *flags = ((*flags | VM_EXEC) ^ VM_EXEC);
      if (cursor->info.spoofed_prot & VM_MAYSHARE)
        *flags |= VM_MAYSHARE;
      else
        *flags = ((*flags | VM_MAYSHARE) ^ VM_MAYSHARE);
    }
    if (cursor->info.need_to_spoof_pgoff) {
      *pgoff = cursor->info.spoofed_pgoff;
    }
    SUSFS_LOGI(
        "spoofing maps -> is_statically: '%d', compare_mode: '%d', is_file: "
        "'%d', is_isolated_entry: '%d', prev_target_ino: '%lu', "
        "next_target_ino: '%lu', target_ino: '%lu', target_dev: '0x%x', "
        "target_pgoff: '0x%x', target_prot: '0x%x', target_addr_size: '0x%x', "
        "spoofed_pathname: '%s', spoofed_ino: '%lu', spoofed_dev: '0x%x', "
        "spoofed_pgoff: '0x%x', spoofed_prot: '0x%x'\n",
        cursor->info.is_statically, cursor->info.compare_mode,
        cursor->info.is_file, cursor->info.is_isolated_entry,
        cursor->info.prev_target_ino, cursor->info.next_target_ino,
        cursor->info.target_ino, cursor->info.target_dev,
 ...
 *     seperately unless the other spoofed name is empty of which spoofed_ino is
 * 0, otherwise there will be inconsistent entries between maps and map_files.
 */
  ...
  list_for_each_entry_safe(cursor, temp, &LH_SUS_MAPS_SPOOFER, list) {
    ...
    if (cursor->info.is_statically && cursor->info.compare_mode > 0 &&
      ...
      if (cursor->info.need_to_spoof_pathname) {
        ...
        // Don't need to check buffer size as 'pathname' is allocated with
        // 'PAGE_SIZE' which is way bigger than SUSFS_MAX_LEN_PATHNAME
 ...
 *   1. Remove the user write access for spoofed symlink name in
 * /proc/self/map_files
...
int susfs_sus_map_files_instantiate(struct vm_area_struct *vma) {
  ...
  list_for_each_entry_safe(cursor, temp, &LH_SUS_MAPS_SPOOFER, list) {
    ...
    if (!cursor->info.is_statically) {
      ...
      case 2:
        if (target_ino != 0 && cursor->info.target_ino == target_ino &&
            ((cursor->info.target_prot & VM_READ) ==
             (target_flags & VM_READ)) &&
            ((cursor->info.target_prot & VM_WRITE) ==
             (target_flags & VM_WRITE)) &&
            ((cursor->info.target_prot & VM_EXEC) ==
             (target_flags & VM_EXEC)) &&
            ((cursor->info.target_prot & VM_MAYSHARE) ==
             (target_flags & VM_MAYSHARE)) &&
            cursor->info.target_addr_size == target_addr_size &&
  ...
  do_spoof:
    if (!(cursor->info.spoofed_ino == 0 ||
          (MAJOR(cursor->info.spoofed_dev) == 0 &&
           (MINOR(cursor->info.spoofed_dev) == 0 ||
            MINOR(cursor->info.spoofed_dev) == 1)))) {
      SUSFS_LOGI(
          "remove user write permission of spoofed symlink '%s' in map_files\n",
          cursor->info.spoofed_pathname);
      return 1;
    } else {
      SUSFS_LOGI("drop dentry of target_ino '%lu' with spoofed_ino '%lu' in "
                 "map_files\n",
  ...
  list_for_each_entry_safe(cursor, temp, &LH_SUS_PROC_FD_LINK, list) {
    if (unlikely(!strcmp(pathname, cursor->info.target_link_name))) {
      SUSFS_LOGI("[uid:%u] spoofing fd link: '%s' -> '%s'\n", current_uid().val,
                 pathname, cursor->info.spoofed_link_name);
...
/* For files/directories in /sdcard/ but not in /sdcard/Android/data/, please
 * delete it by yourself
 */
void susfs_change_error_no_by_pathname(char *const pathname,
                                       int *const errno_to_be_changed,
                                       int const syscall_family) {
  if (!strncmp(pathname, "/system/", 8) || !strncmp(pathname, "/vendor/", 8)) {
...
void __init susfs_init(void) {
  ...
  susfs_my_uname_init();
  SUSFS_LOGI("susfs version: %s, variant: %s\n", SUSFS_VERSION, SUSFS_VARIANT);
}
