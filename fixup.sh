...
if [ "$LAYOUT" == "OLD" ]; then
    ...
    # ksu.c: Make sure susfs.h is included
    if [ -f "$INIT_C" ] && ! grep -q 'linux/susfs.h' "$INIT_C" 2>/dev/null; then
        sed -i '/#include <linux\/fs.h>/a\
#include <linux\/susfs.h>' "$INIT_C"
        echo "[SUSFS-Fixup] ksu.c: Added susfs.h include"
    fi

    # ksud.c: Revert catastrophic patch damage on OLD layout (MamboSU uses native hooks)
